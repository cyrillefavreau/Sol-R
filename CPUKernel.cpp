...
CPUKernel::CPUKernel(bool activeLogging, int optimalNbOfPrimmitivesPerBox,
...
void CPUKernel::mandelbrotSet(const Primitive &primitive, const float x,
  ...
  float maxIterations =
      (float)(NB_MAX_ITERATIONS + m_sceneInfo.pathTracingIteration.x);
...
float4 CPUKernel::sphereUVMapping(const Primitive &primitive,
  ...
  len = sqrt(intersection.x * intersection.x + intersection.y * intersection.y +
...
float4 CPUKernel::triangleUVMapping(const Primitive &primitive,
  ...
  if (u >= 0 && u < material.textureMapping.x && v >= 0 &&
    ...
    case TEXTURE_MANDELBROT:
      mandelbrotSet(primitive, static_cast<float>(u), static_cast<float>(v),
                    result);
      break;
    case TEXTURE_JULIA:
...
float4 CPUKernel::cubeMapping(const Primitive &primitive,
  ...
  if (primitive.type.x == ptCamera) {
    ...
    int y =
        static_cast<int>(KINECT_COLOR_HEIGHT -
    ...
    if (x >= 0 && x < KINECT_COLOR_WIDTH && y >= 0 && y < KINECT_COLOR_HEIGHT) {
      ...
      index = index % (material.textureMapping.x * material.textureMapping.y *
...
#endif // USE_KINECT
  {
    int u = ((primitive.type.x == ptCheckboard) ||
             (primitive.type.x == ptXZPlane) || (primitive.type.x == ptXYPlane))
    ...
    int v =
        ((primitive.type.x == ptCheckboard) || (primitive.type.x == ptXZPlane))
            ? static_cast<int>(intersection.z + primitive.p0.z +
                               primitive.size.z)
...
bool CPUKernel::boxIntersection(const BoundingBox &box, const Ray &ray,
  ...
  tmax =
      (box.parameters[1 - ray.signs.x].x - ray.origin.x) * ray.inv_direction.x;
  tymin = (box.parameters[ray.signs.y].y - ray.origin.y) * ray.inv_direction.y;
  tymax =
      (box.parameters[1 - ray.signs.y].y - ray.origin.y) * ray.inv_direction.y;
  ...
  tzmax =
      (box.parameters[1 - ray.signs.z].z - ray.origin.z) * ray.inv_direction.z;
...
bool CPUKernel::sphereIntersection(const Primitive &sphere, const Ray &ray,
  ...
  if (m_hMaterials[sphere.materialId.x].attributes.y == 0) {
    ...
    newCenter.z =
        sphere.p0.z +
        0.008f * sphere.size.z * sin(cos(m_sceneInfo.misc.y + intersection.z));
...
/*
r = dot(dir,normal);
shadowIntensity = (m_hMaterials[sphere.materialId.x].transparency.x != 0.f) ?
(1.f-fabs(r)) : 1.f;
*/

#if EXTENDED_FEATURES
  // Power textures
  if (m_hMaterials[sphere.materialId.x].textureInfo.y != TEXTURE_NONE &&
      m_hMaterials[sphere.materialId.x].transparency.x != 0) {
    Vertex color =
        sphereUVMapping(sphere, m_hMaterials, textures, intersection, timer);
...
bool CPUKernel::cylinderIntersection(const Primitive &cylinder, const Ray &ray,
  ...
  if (in < -EPSILON) {
    ...
    if (out < -EPSILON) {
      ...
      shadowIntensity = (m_hMaterials[cylinder.materialId.x].transparency.x !=
      0.f) ? (1.f-fabs(r)) : 1.f;
      */
      return true;
    }
...
bool CPUKernel::planeIntersection(const Primitive &primitive, const Ray &ray,
    ...
    if (reverted * ray.origin.y < 0.f &&
      ...
      if (m_hMaterials[primitive.materialId.x].attributes.z == 2) // Wireframe
        collision &= wireFrameMapping(
            ...
            m_hMaterials[primitive.materialId.x].attributes.w, primitive);
    }
  ...
  if (collision) {
    // Shadow intensity
    shadowIntensity =
        1.f; // m_sceneInfo.shadowIntensity.x*(1.f-m_hMaterials[primitive.materialId.x].transparency.x);

    float4 color = m_hMaterials[primitive.materialId.x].color;
    if (primitive.type.x == ptCamera ||
        m_hMaterials[primitive.materialId.x].textureMapping.z != TEXTURE_NONE) {
...
bool CPUKernel::triangleIntersection(const Primitive &triangle, const Ray &ray,
  ...
  // the intersection point with respect to T′.
  if ((a + b) > 1.f) {
    Vertex E23 = {triangle.p0.x - triangle.p1.x, triangle.p0.y - triangle.p1.y,
                  triangle.p0.z - triangle.p1.z};
    Vertex E21 = {triangle.p1.x - triangle.p1.x, triangle.p1.y - triangle.p1.y,
  ...
  while (cptBoxes < m_nbActiveBoxes[m_frame]) {
    ...
    if (boxIntersection(box, r, 0.f, minDistance)) {
      ...
      if (m_sceneInfo.renderBoxes.x != 0) {
        colorBox.x +=
            m_hMaterials[box.startIndex.x % NB_MAX_MATERIALS].color.x / 50.f;
        colorBox.y +=
            m_hMaterials[box.startIndex.x % NB_MAX_MATERIALS].color.y / 50.f;
        colorBox.z +=
            m_hMaterials[box.startIndex.x % NB_MAX_MATERIALS].color.z / 50.f;
      } else {
        // Intersection with primitive within boxes
        for (int cptPrimitives = 0; cptPrimitives < box.nbPrimitives.x;
             ++cptPrimitives) {
          Primitive &primitive =
              m_hPrimitives[box.startIndex.x + cptPrimitives];
          Material &material = m_hMaterials[primitive.materialId.x];
          if (material.attributes.x == 0 ||
              (material.attributes.x == 1 &&
               currentMaterialId != primitive.materialId.x)) // !!!! TEST SHALL
                                                             // BE REMOVED TO
                                                             // INCREASE
                                                             // TRANSPARENCY
                                                             // QUALITY !!!
          {
            ...
            case ptEnvironment:
            case ptSphere: {
              i = sphereIntersection(primitive, r, intersection, normal,
                                     shadowIntensity, back);
              break;
            }
            case ptCylinder: {
              i = cylinderIntersection(primitive, r, intersection, normal,
                                       shadowIntensity, back);
              break;
            }
            case ptEllipsoid: {
              i = ellipsoidIntersection(primitive, r, intersection, normal,
                                        shadowIntensity, back);
              break;
            }
            case ptTriangle: {
              back = false;
              i = triangleIntersection(primitive, r, intersection, normal,
                                       areas, shadowIntensity, back);
              break;
            }
            default: {
              back = false;
              i = planeIntersection(primitive, r, intersection, normal,
...
float CPUKernel::processShadows(const Vertex &lampCenter,
  ...
  while (result < m_sceneInfo.shadowIntensity.x &&
    ...
    if (boxIntersection(box, r, 0.f, m_sceneInfo.viewDistance.x)) {
      int cptPrimitives = 0;
      while (result < m_sceneInfo.shadowIntensity.x &&
             cptPrimitives < box.nbPrimitives.x) {
        Vertex intersection = {0.f, 0.f, 0.f};
        Vertex normal = {0.f, 0.f, 0.f};
        Vertex areas = {0.f, 0.f, 0.f};
        float shadowIntensity = 0.f;
        ...
        if (primitive.index.x != objectId &&
            m_hMaterials[primitive.materialId.x].attributes.x == 0) {

          bool hit = false;
          bool back;
          switch (primitive.type.x) {
          case ptSphere:
            hit = sphereIntersection(primitive, r, intersection, normal,
                                     shadowIntensity, back);
            break;
          case ptEllipsoid:
            hit = ellipsoidIntersection(primitive, r, intersection, normal,
                                        shadowIntensity, back);
            break;
          case ptCylinder:
            hit = cylinderIntersection(primitive, r, intersection, normal,
                                       shadowIntensity, back);
            break;
          case ptTriangle:
            hit = triangleIntersection(primitive, r, intersection, normal,
                                       areas, shadowIntensity, back);
            break;
          case ptCamera:
            hit = false;
            break;
          default:
            hit = planeIntersection(primitive, r, intersection, normal,
                                    shadowIntensity, false);
            break;
          }
          ...
          if (hit) {
            ...
            if (l > EPSILON && l < vectorLength(O_L)) {
              ...
              if (m_hMaterials[primitive.materialId.x].transparency.x != 0.f) {
                ...
                float r =
                    (m_hMaterials[primitive.materialId.x].transparency.x == 0.f)
                        ? 1.f
                        : (1.f -
                           0.8f *
                               m_hMaterials[primitive.materialId.x]
                                   .transparency.x);
                ratio *= r * a;
                // Shadow color
                color.x +=
                    ratio *
                    (0.3f -
                     0.3f * m_hMaterials[primitive.materialId.x].color.x);
                color.y +=
                    ratio *
                    (0.3f -
                     0.3f * m_hMaterials[primitive.materialId.x].color.y);
                color.z +=
                    ratio *
                    (0.3f -
                     0.3f * m_hMaterials[primitive.materialId.x].color.z);
              }
              result += ratio;
            }
            it++;
  ...
  result = (result > m_sceneInfo.shadowIntensity.x)
               ? m_sceneInfo.shadowIntensity.x
               : result;
...
float4 CPUKernel::primitiveShader(
  ...
  if (m_sceneInfo.graphicsLevel.x > 0) {
    ...
    int nbLamps =
        processStdLigths ? m_lightInformationSize : pathTracingInformationSize;
    ...
    for (int cpt = 0; cpt < nbLamps; ++cpt) {
      ...
      if (li.attribute.x != primitive.index.x) {
        ...
        if (li.attribute.x >= 0 &&
          ...
          center.x += m_hMaterials[lamp.materialId.x].innerIllumination.y *
                      m_hRandoms[t] * m_sceneInfo.pathTracingIteration.x /
                      float(m_sceneInfo.maxPathTracingIterations.x);
          center.y += m_hMaterials[lamp.materialId.x].innerIllumination.y *
                      m_hRandoms[t + 1] * m_sceneInfo.pathTracingIteration.x /
                      float(m_sceneInfo.maxPathTracingIterations.x);
          center.z += m_hMaterials[lamp.materialId.x].innerIllumination.y *
                      m_hRandoms[t + 2] * m_sceneInfo.pathTracingIteration.x /
                      float(m_sceneInfo.maxPathTracingIterations.x);
        }

        float4 shadowColor = {0.f, 0.f, 0.f, 0.f};
        if (m_sceneInfo.graphicsLevel.x > 3 && iteration < 4 &&
            m_hMaterials[primitive.materialId.x].innerIllumination.x == 0.f) {
          shadowIntensity = processShadows(center, intersection, li.attribute.x,
        ...
        if (m_sceneInfo.graphicsLevel.x > 0) {
          ...
          // Lambert
          // --------------------------------------------------------------------------------
          float lambert = (m_postProcessingInfo.type.x == ppe_ambientOcclusion)
                              ? 0.6f
                              : dot(normal, lightRay);
          ...
          lambert *= (lambert < 0.f)
                         ? -m_hMaterials[primitive.materialId.x].transparency.x
                         : lambert;
          lambert *= li.color.w;
          ...
          if (m_sceneInfo.graphicsLevel.x > 1 &&
              shadowIntensity < m_sceneInfo.shadowIntensity.x) {
            // --------------------------------------------------------------------------------
            // Blinn - Phong
            // --------------------------------------------------------------------------------
            Vertex viewRay;
            ...
            if (temp != 0.f) {
              ...
              blinnTerm = m_hMaterials[primitive.materialId.x].specular.x *
                          pow(blinnTerm,
                              m_hMaterials[primitive.materialId.x].specular.y);
...
float4 CPUKernel::launchRay(const Ray &ray, Vertex &intersection,
  ...
  int currentMaxIteration =
      (m_sceneInfo.graphicsLevel.x < 3)
          ? 1
          : m_sceneInfo.nbRayIterations.x + m_sceneInfo.pathTracingIteration.x;
...
#endif // PHOTON_ENERGY
  {
    ...
    if (carryon) {
      carryon = intersectionWithPrimitives(
          rayOrigin, iteration, closestPrimitive, closestIntersection, normal,
    ...
    if (carryon) {
      ...
      previousTransparency =
          back ? 1.f
               : m_hMaterials[m_hPrimitives[closestPrimitive].materialId.x]
                     .transparency.x;
#endif // PHOTON_ENERGY
      ...
      colors[iteration] = primitiveShader(
          ...
          areas, iteration, refractionFromColor, shadowIntensity, rBlinn,
      ...
      if (m_hMaterials[m_hPrimitives[closestPrimitive].materialId.x]
              .transparency.x != 0.f) {
      ...
      if (m_sceneInfo.pathTracingIteration.x != 0 &&
          m_hMaterials[m_hPrimitives[closestPrimitive].materialId.x].color.w !=
              0.f) {
        // Randomize view
        float ratio =
            m_hMaterials[m_hPrimitives[closestPrimitive].materialId.x].color.w;
        ratio *= (m_hMaterials[m_hPrimitives[closestPrimitive].materialId.x]
                      .transparency.x == 0.f)
                     ? 1000.f
                     : 1.f;
        int rindex =
            3 * m_sceneInfo.misc.y + m_sceneInfo.pathTracingIteration.x;
  ...
  if (m_sceneInfo.graphicsLevel.x >= 3 &&
      reflectedRays != -1) // TODO: Draft mode should only test
                           // "m_sceneInfo.pathTracingIteration.x==iteration"
  {
    Vertex areas = {0.f, 0.f, 0.f};
    // TODO: Dodgy implementation
    if (intersectionWithPrimitives(
            reflectedRay, reflectedRays, closestPrimitive, closestIntersection,
      ...
      float4 color = primitiveShader(
          ...
          areas, reflectedRays, refractionFromColor, shadowIntensity, rBlinn,
          pathTracingInformation, pathTracingInformationSize, isLightRay);
  ...
  if (!isLightRay) {
    ...
    if (m_hMaterials[primitive.materialId.x].attributes.z == 0) // Wireframe
    {
      ...
      // Photon energy
      // --------------------------------------------------
      intersectionColor.x *= (photonDistance > 0.f)
                                 ? (photonDistance / m_sceneInfo.viewDistance.x)
                                 : 0.f;
      intersectionColor.y *= (photonDistance > 0.f)
                                 ? (photonDistance / m_sceneInfo.viewDistance.x)
                                 : 0.f;
      intersectionColor.z *= (photonDistance > 0.f)
                                 ? (photonDistance / m_sceneInfo.viewDistance.x)
                                 : 0.f;
#endif // PHOTON_ENERGY
      ...
      if (m_sceneInfo.misc.z == 1 && len > D1) {
        ...
        float b = 1.f - (a / D2);
        intersectionColor.x =
            intersectionColor.x * b + m_sceneInfo.backgroundColor.x * (1.f - b);
        intersectionColor.y =
            intersectionColor.y * b + m_sceneInfo.backgroundColor.y * (1.f - b);
        intersectionColor.z =
            intersectionColor.z * b + m_sceneInfo.backgroundColor.z * (1.f - b);
      }
    }
...
#pragma omp parallel for
  for (x = 0; x < m_sceneInfo.size.x; ++x) {
    for (int y = 0; y < m_sceneInfo.size.y; ++y) {
      ...
      bool pathTracing(m_sceneInfo.pathTracingIteration.x >=
                           NB_MAX_ITERATIONS &&
                       m_sceneInfo.pathTracingIteration.x % 2 == 0);
      ...
      if (m_sceneInfo.pathTracingIteration.x == 0) {
        ...
        if (pathTracing) {
          ...
          int rindex = 3 * (index + m_sceneInfo.pathTracingIteration.x +
                            m_sceneInfo.misc.y);
          rindex = rindex % (m_sceneInfo.size.x * m_sceneInfo.size.y - 3);
          lightRay.direction.x +=
              1000.f * m_hRandoms[rindex] * m_postProcessingInfo.param1.x;
          lightRay.direction.y +=
              1000.f * m_hRandoms[rindex + 1] * m_postProcessingInfo.param1.x;
          lightRay.direction.z +=
              1000.f * m_hRandoms[rindex + 2] * m_postProcessingInfo.param1.x;

          // Launch light ray
          launchRay(lightRay, intersection, dof, m_hPrimitivesXYIds[index],
                    pathTracingInformation, pathTracingInformationSize, true);
        }
        ...
        ]*m_postProcessingBuffer[index].w*m_postProcessingInfo.param1.x;//*float(sceneInfo.pathTracingIteration.x)/float(sceneInfo.maxPathTracingIterations.x);
                  ray.origin.y +=
        m_hRandoms[rindex+1]*m_postProcessingBuffer[index].w*m_postProcessingInfo.param1.x;//*float(sceneInfo.pathTracingIteration.x)/float(sceneInfo.maxPathTracingIterations.x);
                  ray.origin.z +=
        m_hRandoms[rindex+2]*m_postProcessingBuffer[index].w*m_postProcessingInfo.param1.x;//*float(sceneInfo.pathTracingIteration.x)/float(sceneInfo.maxPathTracingIterations.x);
      ...
      if (!pathTracing) {
        ...
        {
          ray.direction.x =
              ray.origin.z * 0.001f * (float)(x - (m_sceneInfo.size.x / 2));
          ray.direction.y =
              -ray.origin.z * 0.001f * (float)(y - (m_sceneInfo.size.y / 2));
          ...
          ray.direction.x =
              ray.direction.x - step.x * (float)(x - (m_sceneInfo.size.x / 2));
          ray.direction.y =
              ray.direction.y + step.y * (float)(y - (m_sceneInfo.size.y / 2));
        }
      ...
      if (antialiasingActivated) {
        ...
        for (int I = 0; I < 4; ++I) {
          ...
          float4 c =
              launchRay(r, intersection, dof, m_hPrimitivesXYIds[index],
      ...
      if (m_sceneInfo.pathTracingIteration.x == 0) {
        ...
        ray.origin.x +=
            ...
            m_postProcessingInfo.param1
                .x; //*float(sceneInfo.pathTracingIteration.x)/float(sceneInfo.maxPathTracingIterations.x);
        ray.origin.y +=
  ...
  for (x = 0; x < m_sceneInfo.size.x; ++x) {
    for (int y(0); y < m_sceneInfo.size.y; ++y) {
      ...
      if (x < halfWidth) {
        ...
        eyeRay.direction.x =
            m_viewDir.x -
            step.x * (float)(x - (m_sceneInfo.size.x / 2) + halfWidth / 2);
        eyeRay.direction.y =
            m_viewDir.y + step.y * (float)(y - (m_sceneInfo.size.y / 2));
        eyeRay.direction.z = m_viewDir.z;
      } else {
...
#pragma omp parallel for
  for (x = 0; x < m_sceneInfo.size.x; ++x) {
    for (int y(0); y < m_sceneInfo.size.y; ++y) {
      ...
      if (m_sceneInfo.pathTracingIteration.x > NB_MAX_ITERATIONS) {
        localColor.x /=
            (float)(m_sceneInfo.pathTracingIteration.x - NB_MAX_ITERATIONS + 1);
        localColor.y /=
            (float)(m_sceneInfo.pathTracingIteration.x - NB_MAX_ITERATIONS + 1);
        localColor.z /=
            (float)(m_sceneInfo.pathTracingIteration.x - NB_MAX_ITERATIONS + 1);
...
#pragma omp parallel for
  for (x = 0; x < m_sceneInfo.size.x; ++x) {
    for (int y(0); y < m_sceneInfo.size.y; ++y) {
      int index = y * m_sceneInfo.size.x + x;
      float depth =
          m_postProcessingInfo.param2.x * m_postProcessingBuffer[index].w;
...
#pragma omp parallel for
  for (x = 0; x < m_sceneInfo.size.x; ++x) {
    for (int y(0); y < m_sceneInfo.size.y; ++y) {
      ...
      int div =
          (m_sceneInfo.pathTracingIteration.x > NB_MAX_ITERATIONS)
              ? (m_sceneInfo.pathTracingIteration.x - NB_MAX_ITERATIONS + 1)
              : 1;
      ...
      for (int i = 0; i < m_postProcessingInfo.param3.x; ++i) {
        ...
        int iy = (i + m_sceneInfo.size.x) % wh;
        int xx = x + static_cast<int>(m_hRandoms[ix] *
                                      m_postProcessingInfo.param2.x / 10.f);
        int yy = y + static_cast<int>(m_hRandoms[iy] *
                                      m_postProcessingInfo.param2.x / 10.f);
        localColor.x += m_postProcessingBuffer[index].x;
        localColor.y += m_postProcessingBuffer[index].y;
        localColor.z += m_postProcessingBuffer[index].z;
        if (xx >= 0 && xx < m_sceneInfo.size.x && yy >= 0 &&
            yy < m_sceneInfo.size.y) {
          int localIndex = yy * m_sceneInfo.size.x + xx;
          localColor.x += (localIndex >= 0 && localIndex < wh)
                              ? div * m_hPrimitivesXYIds[localIndex].z / 255
                              : 0.f;
          localColor.y += (localIndex >= 0 && localIndex < wh)
                              ? div * m_hPrimitivesXYIds[localIndex].z / 255
                              : 0.f;
          localColor.z += (localIndex >= 0 && localIndex < wh)
                              ? div * m_hPrimitivesXYIds[localIndex].z / 255
                              : 0.f;
        }
      }
...
void CPUKernel::render_end() {
  if (m_sceneInfo.misc.x == 0) {
    ...
    ::glTexImage2D(GL_TEXTURE_2D, 0, 3, m_sceneInfo.size.x, m_sceneInfo.size.y,
                   0, GL_RGB, GL_UNSIGNED_BYTE, m_bitmap);
    ::glBegin(GL_QUADS);
