...
void MetaballsScene::doAnimate()
{
    ...
    for( int i(0); i<numMetaballs; ++i )
    {
        ...
        case 1:
            ...
            break;
        case 2:
            metaballs[i].position.x=center.x+ratio*amplitude.x*(float)cos(timer/(820+numMetaballs)) - 0.2f*(float)sin(timer/600);
            metaballs[i].position.y=center.z+ratio*amplitude.y*(float)sin(timer/(512+numMetaballs)) - 0.2f*(float)sin(timer/400);
            metaballs[i].position.z=center.y+fabs(ratio*amplitude.z*(float)sin(cos(timer/(450+i))) - c);
            break;
        case 3:
            metaballs[i].position.x=center.x+ratio*amplitude.x*(float)cos(timer/(1200+numMetaballs)) - 0.4f*(float)sin(timer/1250);
            metaballs[i].position.y=center.z+ratio*amplitude.y*(float)sin(timer/(2120+numMetaballs)) - 0.4f*(float)sin(timer/640);
            metaballs[i].position.z=center.y+fabs(ratio*amplitude.z*(float)sin(cos(timer/(450+i))) - c);
            break;
        }
#endif // USE_LEAPMOTION
...
#pragma omp parallel
   ...
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptXYPlane); m_gpuKernel->setPrimitive(m_nbPrimitives,0.f,-4000.f, r.z,r.x,r.y,0.f,m_gpuKernel->getCurrentMaterial()+2);
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptXYPlane); m_gpuKernel->setPrimitive(m_nbPrimitives,0.f,-4000.f,-r.z,r.x,r.y,0.f,m_gpuKernel->getCurrentMaterial()+2);
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptYZPlane); m_gpuKernel->setPrimitive(m_nbPrimitives, r.x,-4000.f,0.f,0.f,r.y,r.z,m_gpuKernel->getCurrentMaterial()+2);
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptYZPlane); m_gpuKernel->setPrimitive(m_nbPrimitives,-r.x,-4000.f,0.f,0.f,r.y,r.z,m_gpuKernel->getCurrentMaterial()+2);
   }
   ...
   {
   Vertex r={size.x*scale.x*0.6f,size.y*scale.y*0.4f,size.z*scale.z*0.6f};
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptXZPlane); m_gpuKernel->setPrimitive(m_nbPrimitives, r.x,-4000.f+r.y,0.f,size.x*scale.x*0.02f,0.f,r.z*1.03f,m_gpuKernel->getCurrentMaterial()+2);
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptXZPlane); m_gpuKernel->setPrimitive(m_nbPrimitives,-r.x,-4000.f+r.y,0.f,size.x*scale.x*0.02f,0.f,r.z*1.03f,m_gpuKernel->getCurrentMaterial()+2);
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptXZPlane); m_gpuKernel->setPrimitive(m_nbPrimitives, 0.f,-4000.f+r.y, r.z,r.x,0.f,size.z*scale.z*0.02f,m_gpuKernel->getCurrentMaterial()+2);
   m_nbPrimitives = m_gpuKernel->addPrimitive(ptXZPlane); m_gpuKernel->setPrimitive(m_nbPrimitives, 0.f,-4000.f+r.y,-r.z,r.x,0.f,size.z*scale.z*0.02f,m_gpuKernel->getCurrentMaterial()+2);
   }
   */
    m_nbPrimitives = m_gpuKernel->addPrimitive(ptSphere ); m_gpuKernel->setPrimitive(  m_nbPrimitives, 10000.f, 1000.f, 10000.f, 3000.f, 0.f, 0.f, m_gpuKernel->getCurrentMaterial()+1);
...
#else
    //m_nbPrimitives = m_gpuKernel->addPrimitive(  ptSphere ); m_gpuKernel->setPrimitive(  m_nbPrimitives, 18000.f*cos(m_timer/50.f), 10000.f+5000.f*cos(m_timer/80.f), -18000.f*sin(m_timer/120.f), 500.f, 0.f, 0.f, DEFAULT_LIGHT_MATERIAL); m_gpuKernel->setPrimitiveIsMovable(  m_nbPrimitives, false );
    m_nbPrimitives = m_gpuKernel->addPrimitive(  ptSphere ); m_gpuKernel->setPrimitive(  m_nbPrimitives, -10000.f, 10000.f, -10000.f, 500.f, 0.f, 0.f, DEFAULT_LIGHT_MATERIAL); m_gpuKernel->setPrimitiveIsMovable(  m_nbPrimitives, false );
#endif // USE_KINECT
