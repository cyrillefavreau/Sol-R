...
void TrefoilKnotScene::trefoilKnot(float R, float t, FLOAT4& p)
{
...
void TrefoilKnotScene::torus(float R, float t, FLOAT4& p )
{
...
void TrefoilKnotScene::star(float R, float t, FLOAT4& p )
{
...
void TrefoilKnotScene::spring(float R, float t, FLOAT4& p)
{
   p.x = R*cos(t);
   p.y = R*sin(t);
   p.z = R*cos(t);
...
void TrefoilKnotScene::heart(float R, float u, float v, FLOAT4& p)
{
...
void TrefoilKnotScene::thing(float R, float t, FLOAT4 a, FLOAT4& p)
{
...
void TrefoilKnotScene::moebius(float R, float u, float v, float s, float du, float dv, FLOAT4& p )
{
...
void TrefoilKnotScene::doInitialize()
{
   ...
   FLOAT4 a = { 1.f+rand()%4, 1.f+rand()%4, 1.f+rand()%4, 1.f+rand()%4 };
   ...
   case 0: // Moebuis
      {
         U.x = 2*static_cast<float>(M_PI)*s; U.y = (2+2*s)*static_cast<float>(M_PI); U.z = 90.f;
         V.x = -0.4f; V.y = 0.4f; V.z = 1.f;
      }
      break;
   case 5: // Heart 
      {
         U.x = 0.f; U.y = 2.f*static_cast<float>(M_PI)*s; U.z = 90.f;
         V.x = 0.f; V.y = 1.f /*static_cast<float>(M_PI)*/; V.z = 1.f;
      }
      break;
   default:
      {
         U.x = 0.f; U.y = 2.f*static_cast<float>(M_PI)*s; U.z = 90.f;
         V.x = 0.f; V.y = 1.f; V.z = 1.f;
      }
      break;
   }
...
#if 1
	      m_nbPrimitives = m_gpuKernel->addPrimitive(  ptCylinder ); m_gpuKernel->setPrimitive( m_nbPrimitives, p0.x,p0.y,p0.z,p1.x,p1.y,p1.z,r,0.f,0.f, material+M);
         m_nbPrimitives = m_gpuKernel->addPrimitive(  ptSphere );   m_gpuKernel->setPrimitive( m_nbPrimitives, p1.x,p1.y,p1.z,r,0.f,0.f, material+M);
#else
	      m_nbPrimitives = m_gpuKernel->addPrimitive(  ptTriangle ); 
         m_gpuKernel->setPrimitive( m_nbPrimitives, 10+(element/V.z), p0.x,p0.y,p0.z,p1.x,p1.y,p1.z,p2.x,p2.y,p2.z,0.f,0.f,0.f, material+M);
	      m_nbPrimitives = m_gpuKernel->addPrimitive(  ptTriangle ); 
         m_gpuKernel->setPrimitive( m_nbPrimitives, 10+(element/V.z), p2.x,p2.y,p2.z,p0.x,p0.y,p0.z,p3.x,p3.y,p3.z,0.f,0.f,0.f, material+M);
#endif // 0
         element++;
      }
   }
...
#endif // 0
   m_nbPrimitives = m_gpuKernel->addPrimitive(  ptSphere  ); m_gpuKernel->setPrimitive( m_nbPrimitives, -4000.f,-1000.f,1000.f,1500.f,0.f,0.f, 114); m_gpuKernel->setPrimitiveIsMovable( m_nbPrimitives, false );
   m_nbPrimitives = m_gpuKernel->addPrimitive(  ptSphere  ); m_gpuKernel->setPrimitive( m_nbPrimitives, 4000.f,-1000.f,1000.f,1500.f,0.f,0.f, 113); m_gpuKernel->setPrimitiveIsMovable( m_nbPrimitives, false );
