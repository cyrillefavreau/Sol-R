...
#ifdef WIN32
void RenderString(float x, float y, float z, void *font, const std::string &string, const Vertex &rgb);
#endif
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    case 'b':
    {
        FLOAT4 color = {rand() % 255 / 255.f, rand() % 255 / 255.f, rand() % 255 / 255.f, 0.5f};
...
void motion(int x, int y)
{
    ...
    if (gAutoFocus)
    {
        // Autofocus
        GPUKernel *kernel = gScene->getKernel();
        int index = kernel->getPrimitiveAt(kernel->getSceneInfo().size.x / 2, kernel->getSceneInfo().size.y / 2);
        if (index != -1)
        {
