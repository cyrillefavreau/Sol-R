...
________________________________________________________________________________
*/
__device__ __INLINE__ float4 skyboxMapping(const SceneInfo &sceneInfo,
                                           Material *materials,
  ...
  float c =
      dot(ray.origin, ray.origin) - (sceneInfo.skybox.x * sceneInfo.skybox.x);
  float d = b * b - 2.f * a * c;
...
ellipsoidIntersection(const SceneInfo &sceneInfo, const Primitive &ellipsoid,
                      Material *materials, const Ray &ray, Vertex &intersection,
...
*/
__device__ __INLINE__ bool
sphereIntersection(const SceneInfo &sceneInfo, const Primitive &sphere,
                   Material *materials, const Ray &ray, Vertex &intersection,
                   Vertex &normal, float &shadowIntensity) {
  bool back = false;
  // solve the equation sphere-ray to find the intersections
  ...
  if (materials[sphere.materialId.x].attributes.y == 0) {
    ...
    newCenter.x =
        sphere.p0.x +
        0.008f * sphere.size.x * cos(sceneInfo.misc.y + intersection.x);
    newCenter.y =
        sphere.p0.y +
        0.008f * sphere.size.y * sin(sceneInfo.misc.y + intersection.y);
    newCenter.z =
        sphere.p0.z +
        0.008f * sphere.size.z * sin(cos(sceneInfo.misc.y + intersection.z));
  ...
  // Shadow management
  r = dot(dir, normal);
  shadowIntensity = (materials[sphere.materialId.x].transparency.x != 0.f)
                        ? (1.f - fabs(r))
                        : 1.f;
...
*/
__device__ __INLINE__ bool
cylinderIntersection(const SceneInfo &sceneInfo, const Primitive &cylinder,
                     Material *materials, const Ray &ray, Vertex &intersection,
                     Vertex &normal, float &shadowIntensity) {
...
*/
__device__ __INLINE__ bool
coneIntersection(const SceneInfo &sceneInfo, const Primitive &cone,
                 Material *materials, const Ray &ray, Vertex &intersection,
                 Vertex &normal, float &shadowIntensity) {
  Vertex O_C = ray.origin - cone.p0;
  Vertex dir = ray.direction;
  Vertex n = crossProduct(dir, cone.n1);
  ...
  Vertex O = crossProduct(O_C, cone.n1);
  float t = -dot(O, n) / ln;
  if (t < 0.f)
    return false;
...
*/
__device__ __INLINE__ bool
planeIntersection(const SceneInfo &sceneInfo, const Primitive &primitive,
                  Material *materials, BitmapBuffer *textures, const Ray &ray,
                  Vertex &intersection, Vertex &normal, float &shadowIntensity,
                  bool reverse) {
    ...
    if (reverted * ray.direction.y < 0.f &&
      ...
      if (materials[primitive.materialId.x].attributes.z == 2) // Wireframe
        collision &= wireFrameMapping(
            ...
            materials[primitive.materialId.x].attributes.w, primitive);
    }
  ...
  if (collision) {
    // Shadow intensity
    shadowIntensity =
        1.f; // sceneInfo.shadowIntensity.x*(1.f-materials[primitive.materialId.x].transparency.x);

    float4 color = materials[primitive.materialId.x].color;
    if (primitive.type.x == ptCamera ||
        materials[primitive.materialId.x].textureIds.x != TEXTURE_NONE) {
      Vertex specular = {0.f, 0.f, 0.f}; // TODO?
      Vertex attributes;
      Vertex advancedAttributes;
      color =
          cubeMapping(sceneInfo, primitive, materials, textures, intersection,
...
*/
__device__ __INLINE__ bool
triangleIntersection(const SceneInfo &sceneInfo, const Primitive &triangle,
                     const Ray &ray, Vertex &intersection, Vertex &normal,
  ...
  normal = normalize(
      (triangle.n0 * areas.x + triangle.n1 * areas.y + triangle.n2 * areas.z) /
...
__device__ float4 intersectionShader(
    ...
    BitmapBuffer *textures, Vertex &intersection, const Vertex &areas,
    Vertex &normal, Vertex &specular, Vertex &attributes,
    Vertex &advancedAttributes) {
  ...
  if (sceneInfo.parameters.y == 1) // Extended geometry
  {
    ...
    case ptEllipsoid: {
      if (materials[primitive.materialId.x].textureIds.x != TEXTURE_NONE) {
        colorAtIntersection =
            sphereUVMapping(primitive, materials, textures, intersection,
                            normal, specular, attributes, advancedAttributes);
      }
      break;
    }
    case ptCheckboard: {
      if (materials[primitive.materialId.x].textureIds.x != TEXTURE_NONE) {
        ...
        int x = sceneInfo.viewDistance.x +
                ((intersection.x - primitive.p0.x) / primitive.size.x);
        int z = sceneInfo.viewDistance.x +
    ...
    case ptTriangle: {
      if (materials[primitive.materialId.x].textureIds.x != TEXTURE_NONE) {
        colorAtIntersection = triangleUVMapping(
            sceneInfo, primitive, materials, textures, intersection, areas,
            normal, specular, attributes, advancedAttributes);
      }
      break;
    }
...
__device__ __INLINE__ bool intersectionWithPrimitives(
    ...
    float4 &closestColor, float4 &colorBox, const int currentmaterialId) {
  bool intersections = false;
  float minDistance = (iteration < 2)
                          ? sceneInfo.viewDistance.x
                          : sceneInfo.viewDistance.x / (iteration + 1);
  ...
  while (cptBoxes < nbActiveBoxes) {
    ...
    if (boxIntersection(box, r, 0.f, minDistance)) {
      ...
      {
        colorBox +=
            materials[box.startIndex.x % NB_MAX_MATERIALS].color / 200.f;
      } else {
        ...
        for (int cptPrimitives = 0; cptPrimitives < box.nbPrimitives.x;
                                                             ...
                                                             // QUALITY !!!
          {
            ...
            if (sceneInfo.parameters.y == 1) // Extended geometry
            {
              ...
              case ptEnvironment:
              case ptSphere:
                i = sphereIntersection(sceneInfo, primitive, materials, r,
                                       intersection, normal, shadowIntensity);
                break;
              case ptCylinder:
                i = cylinderIntersection(sceneInfo, primitive, materials, r,
                                         intersection, normal, shadowIntensity);
                break;
              case ptCone:
                ...
                break;
              case ptEllipsoid:
                i = ellipsoidIntersection(sceneInfo, primitive, materials, r,
                                          intersection, normal,
                                          shadowIntensity);
                break;
              case ptTriangle:
                i = triangleIntersection(sceneInfo, primitive, r, intersection,
                                         normal, areas, shadowIntensity, false);
                break;
              default:
                i = planeIntersection(sceneInfo, primitive, materials, textures,
                                      r, intersection, normal, shadowIntensity,
                                      false);
              }
            } else {
              i = triangleIntersection(sceneInfo, primitive, r, intersection,
...
processShadows(const SceneInfo &sceneInfo, BoundingBox *boudingBoxes,
               const int &nbActiveBoxes, Primitive *primitives,
               Material *materials, BitmapBuffer *textures,
               const int &nbPrimitives, const Vertex &lampCenter,
               const Vertex &origin, const int &lightId, const int &iteration,
               float4 &color, const int &objectId) {
  float result = 0.f;
  int cptBoxes = 0;
  color.x = 0.f;
  color.y = 0.f;
  ...
  computeRayAttributes(r);
  float minDistance =
      sceneInfo.viewDistance.x; // length(r.direction); //(iteration<2) ?
                                // sceneInfo.viewDistance.x :
                                // sceneInfo.viewDistance.x/(iteration+1);

  while (result < (sceneInfo.shadowIntensity.x - sceneInfo.backgroundColor.w) &&
         cptBoxes < nbActiveBoxes) {
    BoundingBox &box = boudingBoxes[cptBoxes];
    if (boxIntersection(box, r, 0.f, minDistance)) {
      int cptPrimitives = 0;
      while (result < sceneInfo.shadowIntensity.x &&
        ...
        Primitive &primitive = primitives[box.startIndex.x + cptPrimitives];
        if (primitive.index.x != lightId && primitive.index.x != objectId &&
            materials[primitive.materialId.x].attributes.x == 0) {

          bool hit = false;
          if (sceneInfo.parameters.y == 1) {
            switch (primitive.type.x) {
            case ptSphere:
              hit = sphereIntersection(sceneInfo, primitive, materials, r,
                                       intersection, normal, shadowIntensity);
              break;
            case ptEllipsoid:
              hit =
                  ellipsoidIntersection(sceneInfo, primitive, materials, r,
                                        intersection, normal, shadowIntensity);
              break;
            case ptCylinder:
              hit = cylinderIntersection(sceneInfo, primitive, materials, r,
                                         intersection, normal, shadowIntensity);
              break;
            case ptCone:
              hit = coneIntersection(sceneInfo, primitive, materials, r,
                                     intersection, normal, shadowIntensity);
              break;
            case ptTriangle:
              hit = triangleIntersection(sceneInfo, primitive, r, intersection,
                                         normal, areas, shadowIntensity, true);
              break;
            case ptCamera:
              hit = false;
              break;
            default:
              hit = planeIntersection(sceneInfo, primitive, materials, textures,
                                      r, intersection, normal, shadowIntensity,
                                      false);
              break;
            }
          ...
          if (hit) {
            ...
            if (l > EPSILON && l < length(O_L)) // ??? 0.f
            {
                     ...
                     if( false && materials[primitive.materialId.x].transparency.x != 0.f )
                     {
                        O_L=normalize(O_L);
                     ...
                     result += ratio;
#else
              result += (materials[primitive.materialId.x].transparency.x == 0.f
                             ? 1.f
                             : 0.2f);
              if (materials[primitive.materialId.x].transparency.x != 0.f)
                color +=
                    0.2f *
                    (1.f - materials[primitive.materialId.x].transparency.x) *
                    materials[primitive.materialId.x].color;
#endif // 0
            }
          }
        }
  ...
  result = (result > sceneInfo.shadowIntensity.x) ? sceneInfo.shadowIntensity.x
                                                  : result;
...
__device__ __INLINE__ float4 primitiveShader(
    ...
    const Vertex &origin, Vertex &normal, const int &objectId,
    Vertex &intersection, const Vertex &areas, float4 &closestColor,
    const int &iteration, float4 &refractionFromColor, float &shadowIntensity,
    float4 &totalBlinn, Vertex &attributes) {
    ...
    for (int cpt = 0; cpt < lightInformationSize; ++cpt) {
      int cptLamp =
          (sceneInfo.pathTracingIteration.x >= NB_MAX_ITERATIONS)
              ? (sceneInfo.pathTracingIteration.x % lightInformationSize)
              : 0;
      if (lightInformation[cptLamp].attribute.x != primitive.index.x) {
        ...
        if (lightRayLength < m.innerIllumination.z) {
          ...
          // float lambert = material.innerIllumination.x +
          // (sceneInfo.graphicsLevel.x>=4 ? dot(normal,lightRay):
          // 0.5f+0.5*dot(normal,initialNormal));

          if (lambert > 0.f && sceneInfo.graphicsLevel.x > 3 &&
            ...
            shadowIntensity = processShadows(
                sceneInfo, boundingBoxes, nbActiveBoxes, primitives, materials,
                textures, nbActivePrimitives, center, intersection,
                lightInformation[cptLamp].attribute.x, iteration, shadowColor,
                objectId);
          }

          lampsColor += shadowColor;
          if (material.attributes.z == 2) {
            lampsColor +=
                (1.f - shadowIntensity) * lightInformation[cptLamp].color;
          } else {
            if (shadowIntensity < 1.f && sceneInfo.graphicsLevel.x > 0) {
              ...
              lambert *= (lambert < 0.f)
                             ? -materials[primitive.materialId.x].transparency.x
                             : 1.f;
              ...
              if (material.innerIllumination.w != 0.f) {
                ...
                lambert *=
                    (1.f + randoms[t] * material.innerIllumination.w * 100.f);
              }
              ...
              lampsColor += lambert * lightInformation[cptLamp].color;

              if (sceneInfo.graphicsLevel.x > 1 &&
                ...
                if (temp != 0.f) {
                  ...
                  float4 b = lightInformation[cptLamp].color *
                             lightInformation[cptLamp].color.w * blinnTerm;
                  totalBlinn =
                      (length(totalBlinn) > length(b)) ? totalBlinn : b;
      ...
      for (int cptPrimitives = 0; cptPrimitives < box.nbPrimitives.x;
        ...
        if (sceneInfo.parameters.y == 1) // Extended geometry
        {
          ...
          case ptTriangle:
            ...
            break;
          default:
            i = planeIntersection(sceneInfo, primitive, materials, textures, r,
                                  intersection, normal, shadowIntensity, false);
            break;
          }
        } else {
          i = triangleIntersection(sceneInfo, primitive, r, intersection,
          ...
          if (dist > postProcessingInfo.param1.x) {
            ...
            if (sceneInfo.graphicsLevel.x != 0) {
              ...
              color = primitiveShader(
                  ...
                  materials, textures, randoms, r.origin, normal,
                  box.startIndex.x + cptPrimitives, intersection, areas,
                  closestColor, 0, refractionFromColor, shadowIntensity, rBlinn,
                  attributes);
            }
            for (int i(0); i < MAXDEPTH; ++i) {
              if (dist < colors[i].w) {
...
________________________________________________________________________________
*/
__device__ __INLINE__ void makeColor(const SceneInfo &sceneInfo, float4 &color,
