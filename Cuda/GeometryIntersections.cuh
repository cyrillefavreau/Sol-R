...
________________________________________________________________________________
*/
__device__ __INLINE__ bool coneIntersection( 
   const SceneInfo& sceneInfo,
            ...
            { 
               ...
               if( material.attributes.x==0 || (material.attributes.x==1 && currentmaterialId!= primitive.materialId.x)) // !!!! TEST SHALL BE REMOVED TO INCREASE TRANSPARENCY QUALITY !!!
               {
                  ...
                  if(sceneInfo.parameters.y==1)  // Extended geometry
                  {
                     ...
                     default          : i = planeIntersection   ( sceneInfo, primitive, materials, textures, r, intersection, normal, shadowIntensity, false);
                     }
                  }
                  else
                  {
   ...
   const int&    objectId)
{
   ...
   computeRayAttributes( r );
   float minDistance=sceneInfo.viewDistance.x; //length(r.direction); //(iteration<2) ? sceneInfo.viewDistance.x : sceneInfo.viewDistance.x/(iteration+1);
   ...
   while( result<(sceneInfo.shadowIntensity.x-sceneInfo.backgroundColor.w) && cptBoxes<nbActiveBoxes )
   {
      ...
      if( boxIntersection(box, r, 0.f, minDistance))
      {
         ...
         while( result<sceneInfo.shadowIntensity.x && cptPrimitives<box.nbPrimitives.x)
         {
            ...
            Primitive& primitive = primitives[box.startIndex.x+cptPrimitives];
            if( primitive.index.x!=lightId && primitive.index.x!=objectId && materials[primitive.materialId.x].attributes.x==0)
            {
               ...
               if( hit )
               {
                  ...
                  if( l>EPSILON && l<length(O_L) ) // ??? 0.f
                  {
                     ...
                     result += ratio;
#else
                     result += (materials[primitive.materialId.x].transparency.x==0.f ? 1.f : 0.2f);
                     if( materials[primitive.materialId.x].transparency.x != 0.f )
                        color += 0.2f*(1.f-materials[primitive.materialId.x].transparency.x)*materials[primitive.materialId.x].color;
#endif // 0
                  }
               }
            }
