...
__device__ __INLINE__ void normalMap(const int &index, const Material &material,
                                     BitmapBuffer *textures, Vertex &normal,
...
__device__ __INLINE__ void bumpMap(const int &index, const Material &material,
                                   BitmapBuffer *textures, Vertex &intersection,
                                   float &value) {
...
__device__ __INLINE__ float4 triangleUVMapping(
  ...
  Vertex T = (primitive.vt0 * areas.x + primitive.vt1 * areas.y +
...
__device__ __INLINE__ float4 sphereUVMapping(
    const Primitive &primitive, Material *materials, BitmapBuffer *textures,
    Vertex &intersection, Vertex &normal, Vertex &specular, Vertex &attributes,
    Vertex &advancedAttributes) {
  Material &material = materials[primitive.materialId.x];
  float4 result = material.color;
...
__device__ __INLINE__ float4 cubeMapping(
    const SceneInfo &sceneInfo, const Primitive &primitive, Material *materials,
    BitmapBuffer *textures, Vertex &intersection, Vertex &normal,
