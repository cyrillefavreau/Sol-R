...
void CudaKernel::render_begin( const float timer )
{
   ...
   if( m_kinectEnabled )
   {
      ...
      if( m_sceneInfo.parameters.w==1 && m_sceneInfo.pathTracingIteration.x==0 ) sceneInfo.graphicsLevel.x = 0;
      if( m_sceneInfo.parameters.w==1 && m_sceneInfo.pathTracingIteration.x==m_sceneInfo.maxPathTracingIterations.x ) sceneInfo.misc.w = 2;
