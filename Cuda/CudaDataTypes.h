...
struct SceneInfo {
  ...
  int1 graphicsLevel; // Graphics level( No Shading=0, Lambert=1, Specular=2,
  ...
  float1 transparentColor; // Value above which r+g+b color is considered as
                           // transparent
  ...
  int1 renderingType; // Rendering type( Standard=0, Anaglyph=1, OculusVR=2,
                      // FishEye=3)
  int1 renderBoxes;   // Activate bounding box rendering( off=0, on=1 );
  int1 pathTracingIteration;     // Current iteration for current frame
  int1 maxPathTracingIterations; // Maximum number of iterations for current
                                 // frame
