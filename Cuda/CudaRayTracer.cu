   ...
   int&                      currentMaterialId )
{
   ...
   int    closestPrimitive  = -1;
		...
		normal, areas, colorBox, back, currentMaterialId )) 
	{
      ...
      if( closestPrimitive==-1 || closestPrimitive>=nbActivePrimitives ) return color; // TODO
      ...
      float colorLight=color.x+color.y+color.z;
      ...
      // ----------
		...
		if( materials[primitives[closestPrimitive].materialId.x].transparency.x != 0.f ) 
		{
         ...
         // Child ray
         color += colorContribution*recursiveRay( iteration+1, currentMaxIteration,
            boundingBoxes, nbActiveBoxes, primitives, nbActivePrimitives, lightInformation, lightInformationSize, 
            nbActiveLamps, materials, textures, randoms, reflectedRay, sceneInfo, postProcessingInfo,
            initialRefraction, firstIntersection, primitiveXYId, currentMaterialId );
		}
         ...
         colorContribution = materials[primitives[closestPrimitive].materialId.x].reflection.x;
		   ...
		   if( sceneInfo.pathTracingIteration.x != 0 && materials[primitives[closestPrimitive].materialId.x].color.w != 0.f)
		   {
            ...
            ratio *= (materials[primitives[closestPrimitive].materialId.x].transparency.x==0.f) ? 1000.f : 1.f;
			   ...
			   reflectedRay.direction.x += randoms[rindex  ]*ratio;
			   reflectedRay.direction.y += randoms[rindex+1]*ratio;
			   reflectedRay.direction.z += randoms[rindex+2]*ratio;
		   }
...
#else
		color = sceneInfo.backgroundColor;
	...
	PrimitiveXYIdBuffer& primitiveXYId)
{
   ...
   intersectionColor = recursiveRay( 0, currentMaxIteration,
      boundingBoxes, nbActiveBoxes, primitives, nbActivePrimitives, lightInformation, lightInformationSize, 
      nbActiveLamps, materials, textures, randoms, ray, sceneInfo, postProcessingInfo,
      initialRefraction, firstIntersection, primitiveXYId, currentMaterialId );
