  ...
  PrimitiveXYIdBuffer& primitiveXYId)
{
  ...
  while( iteration<currentMaxIteration && rayLength<sceneInfo.viewDistance.x && carryon ) 
  {
    ...
    if( carryon ) 
    {
      ...
      if( iteration==0 )
      {
        ...
        if( materials[primitives[closestPrimitive].materialId.x].innerIllumination.x==0.f && 
          (sceneInfo.parameters.z==aiAmbientOcclusion || sceneInfo.parameters.z==aiFullGlobalIllumination ))
        {
          // Global illumination
  ...
  if((sceneInfo.parameters.z==aiAmbientOcclusion || sceneInfo.parameters.z==aiFullGlobalIllumination) && 
    sceneInfo.pathTracingIteration.x>=NB_MAX_ITERATIONS )
  {
    if( useGlobalIllumination )
    {
      ...
      {
        colors[0] -= sceneInfo.shadowIntensity.x; // ambient occlusion
        if( sceneInfo.parameters.z!=aiAmbientOcclusion )
        {
          if(primitives[closestPrimitive].materialId.x!=MATERIAL_NONE)
          {
            Material& material=materials[primitives[closestPrimitive].materialId.x];
            pathTracingRatio*=1.f+material.innerIllumination.x;
          }
          else
          {
