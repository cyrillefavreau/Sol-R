...
GPUKernel::GPUKernel(bool activeLogging, int optimalNbOfBoxes)
    : m_counter(0), m_maxPrimitivesPerBox(0),
      m_optimalNbOfBoxes(optimalNbOfBoxes), m_hPrimitives(nullptr),
      m_hMaterials(nullptr), m_hLamps(nullptr), m_hBoundingBoxes(nullptr),
      m_hPrimitivesXYIds(nullptr), m_hRandoms(nullptr), m_nbActiveMaterials(-1),
      m_nbActiveTextures(0), m_lightInformationSize(0),
      m_activeLogging(activeLogging), m_lightInformation(nullptr),
      m_oculus(false), m_pointSize(1.f), m_morph(0.f), m_nbFrames(0),
#if USE_KINECT
      ...
      m_doneWithAdding(false), m_addingIndex(0), m_refresh(true),
      m_bitmap(nullptr), m_GLMode(-1), m_distortion(0.1f), m_frame(0),
      m_currentMaterial(0), m_treeDepth(2) {
...
void GPUKernel::initBuffers() {
  ...
  // Setup CPU resources
  size_t size = NB_MAX_LIGHTINFORMATIONS + 1;
  m_lightInformation = new LightInformation[size];

  size = NB_MAX_MATERIALS;
  m_hMaterials = new Material[size];
  memset(m_hMaterials, 0, size * sizeof(Material));

#ifndef USE_MANAGED_MEMORY
  size = NB_MAX_BOXES;
  m_hBoundingBoxes = new BoundingBox[size];
  memset(m_hBoundingBoxes, 0, size * sizeof(BoundingBox));

  size = NB_MAX_PRIMITIVES;
  m_hPrimitives = new Primitive[size];
  memset(m_hPrimitives, 0, size * sizeof(Primitive));
#endif

  size = NB_MAX_LAMPS;
  m_hLamps = new Lamp[size];
  memset(m_hLamps, 0, size * sizeof(Lamp));

  // Textures
  size = NB_MAX_TEXTURES;
  memset(m_hTextures, 0, size * sizeof(TextureInformation));
...
void GPUKernel::setPrimitive(const int &index, float x0, float y0, float z0,
  ...
  "center (" << x0 << "," << y0 << "," << z0 << ")," <<
  "size (" << w << "," << h << "," << d << ")," <<
  "material (" << materialId << "," << materialPaddingX << "," <<
  materialPaddingY << ")"
  );
  */
  if (index >= 0 && index <= m_primitives[m_frame].size()) {
  ...
  while (it != (m_primitives[m_frame]).end()) {
    ...
    if (simulate) {
      ...
      if (primitive.materialId ==
          DEFAULT_LIGHT_MATERIAL /* !=DEFAULT_LIGHT_MATERIAL && m_hMaterials[primitive.materialId].innerIllumination.x!=0.f*/) {
        ...
        LOG_INFO(3, "[" << m_treeDepth << "] Lamp " << p << " added ("
                        << m_nbActiveLamps[m_frame] << "/" << NB_MAX_LAMPS
...
int GPUKernel::processOutterBoxes(const int boxSize,
  ...
  LOG_INFO(1, "Maximum number of sub-boxes per box=" << maxPrimitivesPerBox
                                                     << " for level "
                                                     << boundingBoxesDepth);
  return static_cast<int>(maxPrimitivesPerBox);
}
...
void GPUKernel::recursiveDataStreamToGPU(const int depth,
  ...
  while (itob != elements.end()) {
    ...
    if (box.primitives.size() != 0 && m_nbActiveBoxes[m_frame] < NB_MAX_BOXES) {
      ...
      m_hBoundingBoxes[boxIndex].nbPrimitives.x =
          (depth == 0) ? static_cast<int>(box.primitives.size()) : 0;
      m_hBoundingBoxes[boxIndex].startIndex.x =
      ...
      if (depth == 0) {
        ...
        while (itp != box.primitives.end()) {
          ...
          if ((*itp) < NB_MAX_PRIMITIVES) {
            ...
            m_hPrimitives[m_nbActivePrimitives[m_frame]].type.x =
                primitive.type;
            ...
            m_hPrimitives[m_nbActivePrimitives[m_frame]].materialId.x =
      ...
      if (primitive.materialId.x != MATERIAL_NONE) {
        ...
        if (material.innerIllumination.x != 0.f) {
          ...
          while (!found && i < m_nbActiveLamps[m_frame]) {
            LOG_INFO(3, "[Box " << (*it).first << "] Lamp " << i << "/"
                                << m_nbActiveLamps[m_frame] << " = "
                                << m_hLamps[i]
                                << ", Primitive index=" << primitive.index.x);
...
void GPUKernel::buildLightInformationFromTexture(unsigned int index) {
  ...
  for (unsigned int i(0); i < nbLights; ++i) {
    ...
    m_hMaterials[lightInformation.attribute.y].innerIllumination.x =
        0.5f / (nbLights + 1);
    m_hMaterials[DEFAULT_LIGHT_MATERIAL].innerIllumination.x =
        0.5f / (nbLights + 1);
