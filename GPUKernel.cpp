...
GPUKernel::GPUKernel(bool activeLogging, bool protein, int platform, int device)
 : m_hPrimitives(0), 
	...
	int   materialPaddingX, int materialPaddingY )
{
   LOG_INFO(3,"GPUKernel::setPrimitive( " << 
	...
	{
		m_hPrimitives[index].center.x   = x0;
		m_hPrimitives[index].center.y   = y0;
		m_hPrimitives[index].center.z   = z0;
		m_hPrimitives[index].center.w   = w;
      ...
      case ptCylinder:
         {
		      m_hPrimitives[index].otherCenter.x   = x1;
		      m_hPrimitives[index].otherCenter.y   = y1;
		      m_hPrimitives[index].otherCenter.z   = z1;
            ...
            // Size
		      m_hPrimitives[index].size.x = 0.f;
		      m_hPrimitives[index].size.y = len * (m_protein ? 0.6f : 1.f); // Protein only
...
void GPUKernel::updateBoundingBox( const int boxId, const int primitiveIndex )
{
   ...
   if( boxId < NB_MAX_BOXES ) 
   {
      ...
      case ptCylinder: 
         {
            corner0 = m_hPrimitives[primitiveIndex].center;
            corner1 = m_hPrimitives[primitiveIndex].otherCenter;
            break;
         }
      default:
         {
            corner0 = m_hPrimitives[primitiveIndex].center;
            corner1 = m_hPrimitives[primitiveIndex].center;
            break;
         }
      }
      ...
      case ptSphere: 
         {
            p0.x -= m_hPrimitives[primitiveIndex].center.w;
            p0.y -= m_hPrimitives[primitiveIndex].center.w;
            p0.z -= m_hPrimitives[primitiveIndex].center.w;

            p1.x += m_hPrimitives[primitiveIndex].center.w;
            p1.y += m_hPrimitives[primitiveIndex].center.w;
            p1.z += m_hPrimitives[primitiveIndex].center.w;
            break;
         }
      default:
         {
   ...
   for( int i(0); i<=m_nbActiveBoxes; ++i )
   {
      ...
      for( int j(0); j<box.nbPrimitives.x; ++j )
      {
         ...
         std::cout << "- - " << 
            ...
            "center = (" << m_hPrimitives[index].center.x << "," << m_hPrimitives[index].center.y << "," << m_hPrimitives[index].center.z << "), " << 
            "otherCenter = (" << m_hPrimitives[index].otherCenter.x << "," << m_hPrimitives[index].otherCenter.y << "," << m_hPrimitives[index].otherCenter.z << ")" <<
            std::endl;
      }
      std::cout << std::endl;
   }
	...
	int boxId, int index, float4 rotationCenter, float4 cosAngles, float4 sinAngles )
{
   LOG_INFO(3,"GPUKernel::rotatePrimitive(" << boxId << "," << index << ")" );
	...
	{
      if( m_hPrimitives[index].type.x == ptSphere || m_hPrimitives[index].type.x == ptCylinder )
      {
         // Rotate Center
         float4 vector;
         vector.x = m_hPrimitives[index].center.x - rotationCenter.x;
         vector.y = m_hPrimitives[index].center.y - rotationCenter.y;
         vector.z = m_hPrimitives[index].center.z - rotationCenter.z;
         ...
         m_hPrimitives[index].center.x = result.x + rotationCenter.x; 
         m_hPrimitives[index].center.y = result.y + rotationCenter.y; 
         m_hPrimitives[index].center.z = result.z + rotationCenter.z;

         if( m_hPrimitives[index].type.x == ptCylinder )
         {
#if 0
            // Rotate Axis
            vector.x = m_hPrimitives[index].axis.x;// - rotationCenter.x;
            vector.y = m_hPrimitives[index].axis.y;// - rotationCenter.y;
            vector.z = m_hPrimitives[index].axis.z;// - rotationCenter.z;
            result = vector; 
            ...
            m_hPrimitives[index].axis.x = result.x;// + rotationCenter.x;
            m_hPrimitives[index].axis.y = result.y;// + rotationCenter.y;
            m_hPrimitives[index].axis.z = result.z;// + rotationCenter.z;
#endif // 0
            // Rotate Other center
            vector.x = m_hPrimitives[index].otherCenter.x - rotationCenter.x;
            vector.y = m_hPrimitives[index].otherCenter.y - rotationCenter.y;
            vector.z = m_hPrimitives[index].otherCenter.z - rotationCenter.z;
            result = vector; 
            /* X axis */ 
            result.y = vector.y*cosAngles.x - vector.z*sinAngles.x; 
            result.z = vector.y*sinAngles.x + vector.z*cosAngles.x; 
            ...
            m_hPrimitives[index].otherCenter.x = result.x + rotationCenter.x;
            m_hPrimitives[index].otherCenter.y = result.y + rotationCenter.y;
            m_hPrimitives[index].otherCenter.z = result.z + rotationCenter.z;

            // Axis
            float4 axis;
            axis.x = m_hPrimitives[index].otherCenter.x - m_hPrimitives[index].center.x;
            axis.y = m_hPrimitives[index].otherCenter.y - m_hPrimitives[index].center.y;
            axis.z = m_hPrimitives[index].otherCenter.z - m_hPrimitives[index].center.z;
            float len = sqrtf( axis.x*axis.x + axis.y*axis.y + axis.z*axis.z );
            axis.x /= len;
            axis.y /= len;
            axis.z /= len;
	...
	float z )
{
   LOG_INFO(3,"GPUKernel::translatePrimitive(" << boxId << "," << index << ")" );
	if( index>=0 && index<=m_nbActivePrimitives) 
	{
		setPrimitive(
			index, boxId,
			m_hPrimitives[index].center.x+x, m_hPrimitives[index].center.y+y, m_hPrimitives[index].center.z+z,
			m_hPrimitives[index].otherCenter.x+x, m_hPrimitives[index].otherCenter.y+y, m_hPrimitives[index].otherCenter.z+x,
	...
	float& w)
{
	if( index>=0 && index<=m_nbActivePrimitives) 
	{
		x = m_hPrimitives[index].center.x;
		y = m_hPrimitives[index].center.y;
		z = m_hPrimitives[index].center.z;
		w = m_hPrimitives[index].center.w;
	}
}
	...
	float w)
{
	if( index>=0 && index<=m_nbActivePrimitives) 
	{
		m_hPrimitives[index].center.x = x;
		m_hPrimitives[index].center.y = y;
		m_hPrimitives[index].center.z = z;
		m_hPrimitives[index].center.w = w;
		m_hPrimitives[index].size.x = w;
      updateBoundingBox( boxId, index );
	}
...
void GPUKernel::saveBitmapToFile( const std::string& filename, char* bitmap )
{
   FILE *f;
   ...
   int w = m_sceneInfo.width.x;
   int h = m_sceneInfo.height.x;
   int filesize = 54 + gColorDepth*w*h;
   ...
   fwrite(bitmap,1,w*h*gColorDepth,f );
   fclose(f);
};
