...
void OpenCLKernel::populateOpenCLInformation() {
  ...
  for (cl_uint platform(0); platform < m_numberOfPlatforms; ++platform) {
    ...
    CHECKSTATUS(clGetPlatformInfo(m_platforms[platform], CL_PLATFORM_VERSION,
                                  MAX_SOURCE_SIZE, buffer, &len));
    buffer[len] = 0;
    ...
    CHECKSTATUS(clGetPlatformInfo(m_platforms[platform], CL_PLATFORM_PROFILE,
                                  MAX_SOURCE_SIZE, buffer, &len));
    buffer[len] = 0;
    LOG_INFO(1, "  Profile    : " << buffer);

    CHECKSTATUS(clGetPlatformInfo(m_platforms[platform], CL_PLATFORM_EXTENSIONS,
                                  MAX_SOURCE_SIZE, buffer, &len));
    buffer[len] = 0;
    ...
    if (clGetDeviceIDs(m_platforms[platform], CL_DEVICE_TYPE_DEFAULT,
      ...
      for (cl_uint device = 0; device < m_numberOfDevices[platform]; ++device) {
        ...
        CHECKSTATUS(clGetDeviceInfo(m_devices[platform][device],
                                    CL_DEVICE_VERSION, sizeof(buffer), buffer,
                                    NULL));
        ...
        CHECKSTATUS(clGetDeviceInfo(m_devices[platform][device],
                                    CL_DRIVER_VERSION, sizeof(buffer), buffer,
                                    NULL));
        ...
        cl_uint values[10];
        CHECKSTATUS(clGetDeviceInfo(m_devices[platform][device],
                                    CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(value),
                                    &value, NULL));
        ...
        LOG_INFO(1, "    CL_DEVICE_MAX_WORK_ITEM_SIZES      : "
        ...
        CHECKSTATUS(clGetDeviceInfo(m_devices[platform][device], CL_DEVICE_TYPE,
...
*/
OpenCLKernel::OpenCLKernel(bool activeLogging, int optimalNbOfPrimmitivesPerBox,
                           int selectedPlatform, int selectedDevice)
    : GPUKernel(activeLogging, optimalNbOfPrimmitivesPerBox),
      m_platform(selectedPlatform), m_device(selectedDevice), m_hContext(0),
      m_hQueue(0), m_dRandoms(0), m_dBitmap(0), m_dTextures(0),
      m_dPrimitives(0), m_dPostProcessingBuffer(0), m_dPrimitivesXYIds(0),
      m_dLamps(0), m_dLightInformation(0), m_preferredWorkGroupSize(0),
      m_kStandardRenderer(0), m_kAnaglyphRenderer(0), m_k3DVisionRenderer(0),
      m_kFishEyeRenderer(0), m_kVolumeRenderer(0), m_kDefault(0),
      m_kDepthOfField(0), m_kAmbientOcclusion(0), m_kRadiosity(0) {
  LOG_INFO(1, "Platform: " << selectedPlatform
  ...
  m_hContext = clCreateContext(NULL, m_numberOfDevices[m_platform],
                               &m_hDeviceId, NULL, NULL, &status);
  CHECKSTATUS(status);
  if (m_hContext)
...
void OpenCLKernel::recompileKernels(const std::string &filename) {
  ...
  try {
    ...
    hProgram =
        clCreateProgramWithSource(m_hContext, 1, (const char **)&kernel_code,
                                  (const size_t *)&len, &status);
    CHECKSTATUS(status);

    LOG_INFO(1, "Build Program");
    status =
        clBuildProgram(hProgram, 0, NULL, "-cl-fast-relaxed-math", NULL, NULL);
    if (status != CL_SUCCESS) {
      size_t length;
      char buffer[MAX_SOURCE_SIZE];
      memset(buffer, 0, MAX_SOURCE_SIZE);
      CHECKSTATUS(clGetProgramBuildInfo(hProgram, m_hDeviceId,
    ...
    m_kStandardRenderer =
        clCreateKernel(hProgram, "k_standardRenderer", &status);
    CHECKSTATUS(status);

    m_kAnaglyphRenderer =
        clCreateKernel(hProgram, "k_anaglyphRenderer", &status);
    CHECKSTATUS(status);

    m_k3DVisionRenderer =
        clCreateKernel(hProgram, "k_3DVisionRenderer", &status);
    CHECKSTATUS(status);
    ...
    m_kAmbientOcclusion =
        clCreateKernel(hProgram, "k_ambientOcclusion", &status);
    CHECKSTATUS(status);
...
void OpenCLKernel::initializeDevice() {
  ...
  m_dVideo = clCreateBuffer(m_hContext, CL_MEM_READ_ONLY,
                            gVideoWidth * gVideoHeight * gKinectColorVideo, 0,
                            &errorCode);
  m_dDepth = clCreateBuffer(m_hContext, CL_MEM_READ_ONLY,
                            gDepthWidth * gDepthHeight * gKinectColorDepth, 0,
                            &errorCode);
#endif // USE_KINECT
}
...
void OpenCLKernel::releaseKernels() {
  ...
  if (m_kCartoon) {
    CHECKSTATUS(clReleaseKernel(m_kCartoon));
    m_kCartoon = 0;
  }
...
void OpenCLKernel::releaseDevice() {
  ...
  if (m_dPrimitives)
    CHECKSTATUS(clReleaseMemObject(m_dPrimitives));
  if (m_dBoundingBoxes)
...
void OpenCLKernel::render_begin(const float timer) {
  ...
  if (m_refresh) {
    ...
    // LOG_INFO(1, "Data sizes [" << m_frame << "]: " << nbBoxes << ", " <<
    // nbPrimitives << ", " << nbMaterials << ", " << nbLamps );

    if (!m_primitivesTransfered) {
      CHECKSTATUS(clEnqueueWriteBuffer(m_hQueue, m_dBoundingBoxes, CL_TRUE, 0,
                                       nbBoxes * sizeof(BoundingBox),
                                       m_hBoundingBoxes, 0, NULL, NULL));
      CHECKSTATUS(clEnqueueWriteBuffer(m_hQueue, m_dPrimitives, CL_TRUE, 0,
                                       nbPrimitives * sizeof(Primitive),
                                       m_hPrimitives, 0, NULL, NULL));
      CHECKSTATUS(clEnqueueWriteBuffer(m_hQueue, m_dLamps, CL_TRUE, 0,
                                       nbLamps * sizeof(Lamp), m_hLamps, 0,
                                       NULL, NULL));
      CHECKSTATUS(clEnqueueWriteBuffer(m_hQueue, m_dLightInformation, CL_TRUE,
                                       0, m_lightInformationSize *
    ...
    if (!m_materialsTransfered) {
      ...
      LOG_INFO(1, "Randoms size=" << m_sceneInfo.size.x << "x"
                                  << m_sceneInfo.size.y << "x"
                                  << sizeof(RandomBuffer));
      ...
      if (totalSize > 0) {
        ...
        for (int i(0); i < m_nbActiveTextures; ++i) {
          if (m_hTextures[i].buffer != nullptr) {
            int textureSize = m_hTextures[i].size.x * m_hTextures[i].size.y *
                              m_hTextures[i].size.z;
            memcpy(tmpTextures + m_hTextures[i].offset, m_hTextures[i].buffer,
                   textureSize);
          }
        }
        LOG_INFO(1, "Creating texture buffer");
        m_dTextures = clCreateBuffer(m_hContext, CL_MEM_READ_ONLY,
                                     totalSize * sizeof(BitmapBuffer), 0, NULL);
        CHECKSTATUS(clEnqueueWriteBuffer(m_hQueue, m_dTextures, CL_TRUE, 0,
                                         totalSize * sizeof(BitmapBuffer),
                                         tmpTextures, 0, NULL, NULL));
        LOG_INFO(1, "Total GPU texture memory allocated: " << totalSize
                                                           << " bytes");
        delete tmpTextures;
      }
      m_texturesTransfered = true;
    ...
    case vtAnaglyph: {
      ...
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 1, sizeof(cl_int),
                                 (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 2, sizeof(cl_int),
                                 (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 3, sizeof(cl_mem),
                                 (void *)&m_dBoundingBoxes));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 4, sizeof(cl_int),
                                 (void *)&nbBoxes));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 5, sizeof(cl_mem),
                                 (void *)&m_dPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 6, sizeof(cl_int),
                                 (void *)&nbPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 7, sizeof(cl_mem),
                                 (void *)&m_dLightInformation));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 8, sizeof(cl_int),
                                 (void *)&m_lightInformationSize));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 9, sizeof(cl_int),
                                 (void *)&nbLamps));
      ...
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 12, sizeof(cl_mem),
                                 (void *)&m_dRandoms));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 13, sizeof(Vertex),
                                 (void *)&m_viewPos));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 14, sizeof(Vertex),
                                 (void *)&m_viewDir));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 15, sizeof(Vertex),
                                 (void *)&m_angles));
      CHECKSTATUS(clSetKernelArg(m_kAnaglyphRenderer, 16, sizeof(SceneInfo),
                                 (void *)&sceneInfo));
      ...
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kAnaglyphRenderer, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    }
    case vt3DVision: {
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 1, sizeof(cl_int),
                                 (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 2, sizeof(cl_int),
                                 (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 3, sizeof(cl_mem),
                                 (void *)&m_dBoundingBoxes));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 4, sizeof(cl_int),
                                 (void *)&nbBoxes));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 5, sizeof(cl_mem),
                                 (void *)&m_dPrimitives));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 6, sizeof(cl_int),
                                 (void *)&nbPrimitives));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 7, sizeof(cl_mem),
                                 (void *)&m_dLightInformation));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 8, sizeof(cl_int),
                                 (void *)&m_lightInformationSize));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 9, sizeof(cl_int),
                                 (void *)&nbLamps));
      ...
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 12, sizeof(cl_mem),
                                 (void *)&m_dRandoms));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 13, sizeof(Vertex),
                                 (void *)&m_viewPos));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 14, sizeof(Vertex),
                                 (void *)&m_viewDir));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 15, sizeof(Vertex),
                                 (void *)&m_angles));
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 16, sizeof(SceneInfo),
                                 (void *)&sceneInfo));
      ...
      CHECKSTATUS(clSetKernelArg(m_k3DVisionRenderer, 19, sizeof(cl_mem),
                                 (void *)&m_dPrimitivesXYIds));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_k3DVisionRenderer, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    }
    case vtFishEye: {
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(
          clSetKernelArg(m_kFishEyeRenderer, 1, sizeof(cl_int), (void *)&zero));
      CHECKSTATUS(
          clSetKernelArg(m_kFishEyeRenderer, 2, sizeof(cl_int), (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 3, sizeof(cl_mem),
                                 (void *)&m_dBoundingBoxes));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 4, sizeof(cl_int),
                                 (void *)&nbBoxes));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 5, sizeof(cl_mem),
                                 (void *)&m_dPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 6, sizeof(cl_int),
                                 (void *)&nbPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 7, sizeof(cl_mem),
                                 (void *)&m_dLightInformation));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 8, sizeof(cl_int),
                                 (void *)&m_lightInformationSize));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 9, sizeof(cl_int),
                                 (void *)&nbLamps));
      ...
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 12, sizeof(cl_mem),
                                 (void *)&m_dRandoms));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 13, sizeof(Vertex),
                                 (void *)&m_viewPos));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 14, sizeof(Vertex),
                                 (void *)&m_viewDir));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 15, sizeof(Vertex),
                                 (void *)&m_angles));
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 16, sizeof(SceneInfo),
                                 (void *)&sceneInfo));
      ...
      CHECKSTATUS(clSetKernelArg(m_kFishEyeRenderer, 19, sizeof(cl_mem),
                                 (void *)&m_dPrimitivesXYIds));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kFishEyeRenderer, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    }
    case vtVolumeRendering: {
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(
          clSetKernelArg(m_kVolumeRenderer, 1, sizeof(cl_int), (void *)&zero));
      CHECKSTATUS(
          clSetKernelArg(m_kVolumeRenderer, 2, sizeof(cl_int), (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 3, sizeof(cl_mem),
                                 (void *)&m_dBoundingBoxes));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 4, sizeof(cl_int),
                                 (void *)&nbBoxes));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 5, sizeof(cl_mem),
                                 (void *)&m_dPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 6, sizeof(cl_int),
                                 (void *)&nbPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 7, sizeof(cl_mem),
                                 (void *)&m_dLightInformation));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 8, sizeof(cl_int),
                                 (void *)&m_lightInformationSize));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 9, sizeof(cl_int),
                                 (void *)&nbLamps));
      ...
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 12, sizeof(cl_mem),
                                 (void *)&m_dRandoms));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 13, sizeof(Vertex),
                                 (void *)&m_viewPos));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 14, sizeof(Vertex),
                                 (void *)&m_viewDir));
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 15, sizeof(Vertex),
                                 (void *)&m_angles));
      ...
      CHECKSTATUS(clSetKernelArg(m_kVolumeRenderer, 19, sizeof(cl_mem),
                                 (void *)&m_dPrimitivesXYIds));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kVolumeRenderer, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    }
    default: {
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 1, sizeof(cl_int),
                                 (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 2, sizeof(cl_int),
                                 (void *)&zero));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 3, sizeof(cl_mem),
                                 (void *)&m_dBoundingBoxes));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 4, sizeof(cl_int),
                                 (void *)&nbBoxes));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 5, sizeof(cl_mem),
                                 (void *)&m_dPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 6, sizeof(cl_int),
                                 (void *)&nbPrimitives));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 7, sizeof(cl_mem),
                                 (void *)&m_dLightInformation));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 8, sizeof(cl_int),
                                 (void *)&m_lightInformationSize));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 9, sizeof(cl_int),
                                 (void *)&nbLamps));
      ...
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 12, sizeof(cl_mem),
                                 (void *)&m_dRandoms));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 13, sizeof(Vertex),
                                 (void *)&m_viewPos));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 14, sizeof(Vertex),
                                 (void *)&m_viewDir));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 15, sizeof(Vertex),
                                 (void *)&m_angles));
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 16, sizeof(SceneInfo),
                                 (void *)&sceneInfo));
      ...
      CHECKSTATUS(clSetKernelArg(m_kStandardRenderer, 19, sizeof(cl_mem),
                                 (void *)&m_dPrimitivesXYIds));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kStandardRenderer, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    }
    ...
    case ppe_depthOfField:
      CHECKSTATUS(clSetKernelArg(m_kDepthOfField, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(clSetKernelArg(m_kDepthOfField, 1, sizeof(SceneInfo),
                                 (void *)&sceneInfo));
      CHECKSTATUS(clSetKernelArg(m_kDepthOfField, 2, sizeof(PostProcessingInfo),
                                 (void *)&m_postProcessingInfo));
      ...
      CHECKSTATUS(clSetKernelArg(m_kDepthOfField, 5, sizeof(cl_mem),
                                 (void *)&m_dBitmap));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kDepthOfField, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    case ppe_ambientOcclusion:
      CHECKSTATUS(clSetKernelArg(m_kAmbientOcclusion, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(clSetKernelArg(m_kAmbientOcclusion, 1, sizeof(SceneInfo),
                                 (void *)&sceneInfo));
      ...
      CHECKSTATUS(clSetKernelArg(m_kAmbientOcclusion, 5, sizeof(cl_mem),
                                 (void *)&m_dBitmap));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kAmbientOcclusion, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    case ppe_radiosity:
      CHECKSTATUS(clSetKernelArg(m_kRadiosity, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(clSetKernelArg(m_kRadiosity, 1, sizeof(SceneInfo),
                                 (void *)&sceneInfo));
      CHECKSTATUS(clSetKernelArg(m_kRadiosity, 2, sizeof(PostProcessingInfo),
                                 (void *)&m_postProcessingInfo));
      ...
      CHECKSTATUS(clSetKernelArg(m_kRadiosity, 4, sizeof(cl_mem),
                                 (void *)&m_dPostProcessingBuffer));
      CHECKSTATUS(
          clSetKernelArg(m_kRadiosity, 5, sizeof(cl_mem), (void *)&m_dRandoms));
      CHECKSTATUS(
          clSetKernelArg(m_kRadiosity, 6, sizeof(cl_mem), (void *)&m_dBitmap));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kRadiosity, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    case ppe_filter:
      CHECKSTATUS(clSetKernelArg(m_kFilter, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(
          clSetKernelArg(m_kFilter, 1, sizeof(SceneInfo), (void *)&sceneInfo));
      CHECKSTATUS(clSetKernelArg(m_kFilter, 2, sizeof(PostProcessingInfo),
                                 (void *)&m_postProcessingInfo));
      CHECKSTATUS(clSetKernelArg(m_kFilter, 3, sizeof(cl_mem),
                                 (void *)&m_dPostProcessingBuffer));
      CHECKSTATUS(
          clSetKernelArg(m_kFilter, 4, sizeof(cl_mem), (void *)&m_dBitmap));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kFilter, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    case ppe_cartoon:
      CHECKSTATUS(clSetKernelArg(m_kCartoon, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(
          clSetKernelArg(m_kCartoon, 1, sizeof(SceneInfo), (void *)&sceneInfo));
      CHECKSTATUS(clSetKernelArg(m_kCartoon, 2, sizeof(PostProcessingInfo),
                                 (void *)&m_postProcessingInfo));
      CHECKSTATUS(clSetKernelArg(m_kCartoon, 3, sizeof(cl_mem),
                                 (void *)&m_dPostProcessingBuffer));
      CHECKSTATUS(
          clSetKernelArg(m_kCartoon, 4, sizeof(cl_mem), (void *)&m_dBitmap));
      CHECKSTATUS(clEnqueueNDRangeKernel(m_hQueue, m_kCartoon, 2, NULL,
                                         szGlobalWorkSize, szLocalWorkSize, 0,
                                         0, 0));
      break;
    default:
      CHECKSTATUS(clSetKernelArg(m_kDefault, 0, sizeof(cl_int2),
                                 (void *)&m_occupancyParameters));
      CHECKSTATUS(
          clSetKernelArg(m_kDefault, 1, sizeof(SceneInfo), (void *)&sceneInfo));
      CHECKSTATUS(clSetKernelArg(m_kDefault, 2, sizeof(PostProcessingInfo),
                                 (void *)&m_postProcessingInfo));
      CHECKSTATUS(clSetKernelArg(m_kDefault, 3, sizeof(cl_mem),
                                 (void *)&m_dPostProcessingBuffer));
      CHECKSTATUS(
          clSetKernelArg(m_kDefault, 4, sizeof(cl_mem), (void *)&m_dBitmap));
  ...
  m_refresh = (m_sceneInfo.pathTracingIteration.x <
               m_sceneInfo.maxPathTracingIterations.x);
}
...
void OpenCLKernel::render_end() {
  ...
  // ------------------------------------------------------------
  size_t size = m_sceneInfo.size.x * m_sceneInfo.size.y * sizeof(BitmapBuffer) *
                gColorDepth;
        ...
        for (float x(0); x < 1; x += step) {
          for (float y(0); y < 1; y += step) {
            ...
            */
            ...
            ::glVertex3f(center.x + 0.5f * p0.x * d0, center.y + p0.y * d0,
                         0.f);

            ::glTexCoord2f(1.f - (b + (x + step) / 2.f), y);
            ::glVertex3f(center.x + 0.5f * p1.x * d1, center.y + p1.y * d1,
                         0.f);

            ::glTexCoord2f(1.f - (b + (x + step) / 2.f), y + step);
            ::glVertex3f(center.x + 0.5f * p2.x * d2, center.y + p2.y * d2,
                         0.f);

            ::glTexCoord2f(1.f - (b + (x / 2.f)), y + step);
            ::glVertex3f(center.x + 0.5f * p3.x * d3, center.y + p3.y * d3,
                         0.f);
            ::glEnd();
          }
        }
...
void OpenCLKernel::reshape() {
  ...
  int errorCode;
  m_dBitmap = clCreateBuffer(
      m_hContext, CL_MEM_READ_WRITE,
      MAX_BITMAP_SIZE * sizeof(BitmapBuffer) * gColorDepth, 0, &errorCode);
  m_dRandoms =
      clCreateBuffer(m_hContext, CL_MEM_READ_ONLY,
                     MAX_BITMAP_SIZE * sizeof(RandomBuffer), 0, &errorCode);
  m_dPostProcessingBuffer = clCreateBuffer(
      m_hContext, CL_MEM_READ_WRITE,
      MAX_BITMAP_SIZE * sizeof(PostProcessingBuffer), 0, &errorCode);
  m_dPrimitivesXYIds = clCreateBuffer(
      m_hContext, CL_MEM_READ_WRITE,
      MAX_BITMAP_SIZE * sizeof(PrimitiveXYIdBuffer), 0, &errorCode);
}

int OpenCLKernel::getNumPlatforms() { return m_numberOfPlatforms; }
