...
typedef struct
{
   ...
   vec4f location;                  // Position in space
   float4 color;                     // Light
} LightInformation;
...
typedef struct
{
   vec4f parameters[2];     // Bottom-Left and Top-Right corners
...
________________________________________________________________________________
*/
static void vectorRefraction(
   vec4f*      refracted, 
   const vec4f incident, 
   const float  n1, 
   const vec4f normal, 
   const float  n2 )
{
...
// Ambient occlusion
// --------------------
static void ambientOcclusionMap(
   const int        index,
   CONST Material*  material,
   CONST BitmapBuffer*    textures,
   ...
   vec4f* advancedAttributes)
{
   ...
   vec4f I=normalize((*intersection)-(*primitive).p0);
...
________________________________________________________________________________
*/
static float4 triangleUVMapping(
   const SceneInfo* sceneInfo,
   CONST Primitive* primitive,
   CONST Material*        materials,
   CONST BitmapBuffer*    textures,
   ...
   vec4f* advancedAttributes)
{
   ...
   vec4f T = ((*primitive).vt0*areas.x+(*primitive).vt1*areas.y+(*primitive).vt2*areas.z)/(areas.x+areas.y+areas.z);
...
________________________________________________________________________________
*/
static bool ellipsoidIntersection(
   const SceneInfo* sceneInfo,
   ...
   float* shadowIntensity) 
{
   ...
   vec4f O_C = (*ray).origin-(*ellipsoid).p0;
   vec4f dir = normalize((*ray).direction);
   ...
   ) 
{
   ...
   float4 result = (*material).color;
   // solve the equation sphere-ray to find the intersections
   vec4f dir = normalize((*ray).direction-(*ray).origin); 
...
________________________________________________________________________________
*/
static bool sphereIntersection(
   const SceneInfo* sceneInfo,
   ...
   ) 
{
   bool back=false;
   // solve the equation sphere-ray to find the intersections
   vec4f O_C = (*ray).origin-(*sphere).p0;
   vec4f dir = normalize((*ray).direction); 
...
static vec4f project( const vec4f A, const vec4f B)
{
...
________________________________________________________________________________
*/
static bool cylinderIntersection(
   const SceneInfo* sceneInfo,
   ...
   float*     shadowIntensity) 
{            
   vec4f O_C = (*ray).origin-(*cylinder).p0;
   vec4f dir = (*ray).direction;
   vec4f n   = cross(dir, (*cylinder).n1);
   float ln = length(n);
   ...
   vec4f HB1 = (*intersection)-(*cylinder).p0;
   vec4f HB2 = (*intersection)-(*cylinder).p1;
   ...
   vec4f V = (*intersection)-(*cylinder).p2;
   ...
   const bool       processingShadows )
{
   ...
   vec4f E01=(*triangle).p1-(*triangle).p0;
   vec4f E03=(*triangle).p2-(*triangle).p0;
   ...
   if ((a+b) > 1.f) 
   {
      vec4f E23 = (*triangle).p0-(*triangle).p1;
   ...
   // Normal
   vec4f v0 = ((*triangle).p0 - (*intersection));
   vec4f v1 = ((*triangle).p1 - (*intersection));
   vec4f v2 = ((*triangle).p2 - (*intersection));
   ...
   if( (*sceneInfo).parameters.x==1 )
   {
      ...
      vec4f N=normalize((*ray).direction);
...
________________________________________________________________________________
*/
static float4 intersectionShader(
   const SceneInfo* sceneInfo,
   CONST Primitive* primitive, 
   CONST Material*  materials,
   CONST BitmapBuffer* textures,
   ...
   float4*       color)
{
   ...
   while( result<(*sceneInfo).shadowIntensity && cptBoxes<nbActiveBoxes )
   {
      ...
      if(boxIntersection(box, &r, 0.05f, minDistance))
      {
         ...
         {
            vec4f intersection = {0.f,0.f,0.f,0.f};
            vec4f normal       = {0.f,0.f,0.f,0.f};
            vec4f areas        = {0.f,0.f,0.f,0.f};
   ...
   vec4f*      attributes)
{
   ...
   // Bump
   vec4f bumpNormal={0.f,0.f,0.f,0.f};
   ...
   if( (*sceneInfo).graphicsLevel==0 || (*material).innerIllumination.x!=0.f || (*material).attributes.z!=0 )
   {
   ...
   if( (*sceneInfo).graphicsLevel>0 )
   {
      ...
      for( int c=0; c<C; ++c ) 
      {
         ...
         if(lightInformation[cptLamp].attribute.x != (*primitive).index)
         {
            ...
            vec4f center=lightInformation[cptLamp].location;
            ...
            vec4f lightRay = center - (*intersection);
            ...
            if( lightRayLength<(*m).innerIllumination.z )
            {
               ...
               if( (*sceneInfo).graphicsLevel>0 )
               {
                  ...
                  if( (*sceneInfo).graphicsLevel>1 && (*shadowIntensity)<(*sceneInfo).shadowIntensity )
                  {
                     ...
                     // Blinn - Phong
                     // --------------------------------------------------------------------------------
                     vec4f viewRay = normalize((*intersection) - origin);
                     vec4f blinnDir = lightRay - viewRay;
   ...
   CONST PrimitiveXYIdBuffer* primitiveXYId)
{
   ...
   vec4f closestIntersection = {0.f,0.f,0.f,0.f};
   vec4f firstIntersection   = {0.f,0.f,0.f,0.f};
   vec4f normal              = {0.f,0.f,0.f,0.f};
   ...
   float  shadowIntensity = 0.f;
   float4 refractionFromColor;
   vec4f reflectedTarget;
   float4 colorBox = {0.f,0.f,0.f,0.f};
   vec4f latestIntersection=(*ray).origin;
   float rayLength=0.f;
   ...
   while( iteration<currentMaxIteration && rayLength<(*sceneInfo).viewDistance && carryon ) 
   {
      ...
      if( carryon ) 
      {
         ...
         if(transparency!=0.f) // Transparency
         {
            ...
            // Actual refraction
            vec4f O_E = normalize(closestIntersection-rayOrigin.origin);
      ...
      else
      {
         ...
         else
         {
            if( (*sceneInfo).parameters.y==1 )
            {
               ...
               vec4f dir = normalize(rayOrigin.direction-rayOrigin.origin);
      ...
      (*sceneInfo).pathTracingIteration>=NB_MAX_ITERATIONS)
   {
      if((*sceneInfo).parameters.z==aiAdvancedGlobalIllumination)
      {
         ...
         {
            //if( (*sceneInfo).parameters.z==aiGlobalIllumination )
            {
                // Ambient occlusion
   ...
   CONST PrimitiveXYIdBuffer*  primitiveXYIds)
{
   ...
   vec4f rotationCenter = {0.f,0.f,0.f,0.f};
