...
void OpenCLKernel::populateOpenCLInformation()
{
    ...
    for (cl_uint platform(0); platform < m_numberOfPlatforms; ++platform)
    {
        ...
        if (clGetDeviceIDs(m_platforms[platform], CL_DEVICE_TYPE_CPU, MAX_DEVICES, m_devices[platform],
...
*/
OpenCLKernel::OpenCLKernel(bool activeLogging, int optimalNbOfPrimmitivesPerBox, int selectedPlatform,
                           int selectedDevice)
...
void OpenCLKernel::recompileKernels(const std::string &filename)
{
    ...
    try
    {
        ...
        if (status != CL_SUCCESS)
        {
            ...
            CHECKSTATUS(
                clGetProgramBuildInfo(hProgram, m_hDeviceId, CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, &length));
...
#if USE_KINECT
    m_dVideo =
        clCreateBuffer(m_hContext, CL_MEM_READ_ONLY, gVideoWidth * gVideoHeight * gKinectColorVideo, 0, &errorCode);
    m_dDepth =
        clCreateBuffer(m_hContext, CL_MEM_READ_ONLY, gDepthWidth * gDepthHeight * gKinectColorDepth, 0, &errorCode);
#endif // USE_KINECT
}
...
// LOG_INFO(1,"Render_begin");
    ...
    if (m_refresh)
    {
        ...
        if (!m_primitivesTransfered)
        {
            ...
            CHECKSTATUS(clEnqueueWriteBuffer(m_hQueue, m_dPrimitives, CL_TRUE, 0, nbPrimitives * sizeof(Primitive),
        ...
        case ppe_radiosity:
            ...
            break;
        case ppe_filter:
            CHECKSTATUS(clSetKernelArg(m_kFilter, 0, sizeof(cl_int2), (void *)&m_occupancyParameters));
