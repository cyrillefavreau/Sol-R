...
void getKernelCode(std::string &str) { str = ""; }
...
void OpenCLKernel::populateOpenCLInformation() {
  ...
  for (cl_uint platform(0); platform < m_numberOfPlatforms; ++platform) {
    ...
    if (clGetDeviceIDs(m_platforms[platform], CL_DEVICE_TYPE_CPU, MAX_DEVICES,
                       m_devices[platform],
...
// LOG_INFO(1,"Render_begin");
  ...
  if (m_refresh) {
    ...
    LOG_INFO(3, "Data sizes [" << m_frame << "]: " << nbBoxes << ", "
                               << nbPrimitives << ", " << m_lightInformationSize
                               << ", " << nbLamps);
    ...
    default:
      ...
      CHECKSTATUS(clSetKernelArg(m_kDefault, 2, sizeof(cl_mem),
                                 (void *)&m_dPostProcessingBuffer));
      CHECKSTATUS(
          clSetKernelArg(m_kDefault, 3, sizeof(cl_mem), (void *)&m_dBitmap));
