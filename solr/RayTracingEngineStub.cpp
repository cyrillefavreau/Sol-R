...
int RayTracer_SetSceneInfo(
        ...
        double bgColorB, double bgColorA, int renderBoxes, int pathTracingIteration,
        int maxPathTracingIterations, int outputType, int timer, int fogEffect,
        int isometric3D, int doubleSidedTriangles, int gradientBackGround,
    ...
    LOG_INFO(3, "RayTracer_SetSceneInfo ("
             ...
             << bgColorB << "," << bgColorA << "," << renderingType << ","
             << renderBoxes << "," << pathTracingIteration << ","
             << maxPathTracingIterations << "," << outputType << ","
...
int RayTracer_InitializeKernel(bool activeLogging, int platform, int device) {
    ...
    if (gKernel == 0) {
        gKernel =
                new GenericGPUKernel(activeLogging, NB_MAX_BOXES, platform, device);
...
void RayTracer_SetCamera(double eye_x, double eye_y, double eye_z, double dir_x,
    ...
    Vertex eye = {static_cast<float>(eye_x), static_cast<float>(eye_y),
                  static_cast<float>(eye_z)};
    Vertex dir = {static_cast<float>(dir_x), static_cast<float>(dir_y),
                  static_cast<float>(dir_z)};
    Vertex angles = {static_cast<float>(angle_x), static_cast<float>(angle_y),
...
int RayTracer_RunKernel(double timer,
    ...
    if (false && gSceneInfoStub.misc.x == otDelphi) {
        BitmapBuffer *dst = new BitmapBuffer[gSceneInfoStub.size.x *
                gSceneInfoStub.size.y * gColorDepth];
        BitmapBuffer *src = gKernel->getBitmap();
        for (int y(0); y < gSceneInfoStub.size.y; ++y) {
            for (int x(0); x < gSceneInfoStub.size.x * gColorDepth;
                 x += gColorDepth) {
                int indexSrc = (y * gSceneInfoStub.size.x * gColorDepth) + x;
                int indexDst =
                        (y + 1) * (gSceneInfoStub.size.x * gColorDepth) - x - gColorDepth;
        ...
        memcpy(image, gKernel->getBitmap(),
               gSceneInfoStub.size.x * gSceneInfoStub.size.y * gColorDepth);
    }
    return 0;
}
...
int RayTracer_SetPrimitive(int index, double p0_x, double p0_y, double p0_z,
    ...
    gKernel->setPrimitive(index, static_cast<float>(p0_x),
                          static_cast<float>(p0_y), static_cast<float>(p0_z),
                          static_cast<float>(p1_x), static_cast<float>(p1_y),
                          static_cast<float>(p1_z), static_cast<float>(p2_x),
                          static_cast<float>(p2_y), static_cast<float>(p2_z),
                          static_cast<float>(size_x), static_cast<float>(size_y),
                          static_cast<float>(size_z), materialId);
    return 0;
}
...
int RayTracer_RotatePrimitive(int index, double rx, double ry, double rz, double ax,
    ...
    FLOAT4 rotationCenter = {static_cast<float>(rx), static_cast<float>(ry),
                             static_cast<float>(rz), 0.f};
    FLOAT4 angles = {static_cast<float>(ax), static_cast<float>(ay),
...
int RayTracer_RotatePrimitives(int fromBoxId, int toBoxId, double rx, double ry,
    ...
    try {
        Vertex rotationCenter = {static_cast<float>(rx), static_cast<float>(ry),
                                 static_cast<float>(rz)};
        Vertex angles = {static_cast<float>(ax), static_cast<float>(ay),
...
int RayTracer_SetPrimitiveNormals(int index, double n0_x, double n0_y, double n0_z,
    ...
    Vertex n0 = {static_cast<float>(n0_x), static_cast<float>(n0_y),
                 static_cast<float>(n0_z)};
    Vertex n1 = {static_cast<float>(n1_x), static_cast<float>(n1_y),
                 static_cast<float>(n1_z)};
    Vertex n2 = {static_cast<float>(n2_x), static_cast<float>(n2_y),
...
int RayTracer_SetPrimitiveTextureCoordinates(
    ...
    Vertex t0 = {static_cast<float>(t0_x), static_cast<float>(t0_y),
                 static_cast<float>(t0_z)};
    Vertex t1 = {static_cast<float>(t1_x), static_cast<float>(t1_y),
                 static_cast<float>(t1_z)};
    Vertex t2 = {static_cast<float>(t2_x), static_cast<float>(t2_y),
                 static_cast<float>(t2_z)};
...
#if USE_KINECT
    Vertex position = {static_cast<float>(p0_x), static_cast<float>(p0_y),
...
int RayTracer_GetMaterial(
    ...
    int returnValue = gKernel->getMaterialAttributes(
                in_index, color_r, color_g, color_b, noise, reflection, refraction,
                procedural, wireframe, wireframeDepth, transparency, opacity,
                diffuseTextureId, normalTextureId, bumpTextureId, specularTextureId,
                reflectionTextureId, transparencyTextureId, ambientOcclusionTextureId,
                specValue, specPower, specCoef, innerIllumination, illuminationDiffusion,
                illuminationPropagation, fastTransparency);
...
int RayTracer_LoadMolecule(char *filename, int geometryType, double defaultAtomSize,
    ...
    Vertex minPos = prbReader.loadAtomsFromFile(
                filename, *gKernel, static_cast<GeometryType>(geometryType),
                static_cast<float>(defaultAtomSize), static_cast<float>(defaultStickSize),
...
int RayTracer_LoadOBJModel(char *filename, int materialId, int autoScale,
    ...
    Vertex minPos = objReader.loadModelFromFile(
                filename, *gKernel, center, (autoScale == 1), objectScale, true,
