...
Vertex MapReader::loadFromFile(const std::string &filename, GPUKernel &kernel) {
      ...
      if (line.length() > 0) {
        ...
        if (line.find("<COLOURSCHEME ") == 0) {
          ...
          if (line.find("TYPE") != -1) {
            ...
            s >> item >> block.north >> item >> block.south >> item >>
                block.east >> item >> block.west >> item >> block.split >>
                item >> block.hollow;
          }
  ...
  while (itj != jewels.end()) {
    ...
    kernel.setPrimitive(nbPrimitives, position.x + blockSize.x / 2.f,
                        ...
                        position.z + blockSize.x / 2.f, blockSize.x / 4.f, 0.f,
                        0.f, jewelMaterial);
    ++itj;
  }
  ...
  while (it != blocks.end()) {
    ...
    case 19: {
      ...
      kernel.setPrimitive(nbPrimitives, position.x, position.y, position.z,
                          dimension.x, position.y, position.z, dimension.x,
                          position.y, dimension.z, 0.f, 0.f, 0.f, zoneMaterial,
                          1, 1);
      nbPrimitives = kernel.addPrimitive(ptTriangle);
      kernel.setPrimitive(nbPrimitives, dimension.x, position.y, dimension.z,
                          position.x, position.y, dimension.z, position.x,
                          position.y, position.z, 0.f, 0.f, 0.f, zoneMaterial,
                          1, 1);
      break;
    }
    default: { break; }
