...
Vertex PDBReader::loadAtomsFromFile(const std::string &filename, GPUKernel &cudaKernel, GeometryType geometryType,
                                    const float defaultAtomSize, const float defaultStickSize, const int materialType,
                                    const Vertex scale, const bool useModels)
{
    for (int i = 0; i < NB_ELEMENTS; ++i)
    {
        ...
        cudaKernel.setMaterial(i, static_cast<float>(colormap[i].R) / 255.f, static_cast<float>(colormap[i].G) / 255.f,
                               ...
                               TEXTURE_NONE, 1.f, 100.f, 0.f,
                               /*(i%3==0) ? 0.5f : 0.f,*/ 0.f, cudaKernel.getSceneInfo().viewDistance, 0.f, false);
    }
    ...
    LOG_INFO(1, "Loaded " << filename << " into frame " << cudaKernel.getFrame() << " ["
                          << cudaKernel.getNbActivePrimitives() << " primitives]");
