        ...
        for (int cptPrimitives = 0; cptPrimitives < box.nbPrimitives;
          ...
          if (material.attributes.x == 0 ||
              (material.attributes.x == 1 &&
               currentmaterialId != primitive.materialId)) // !!!! TEST SHALL BE
                                                           // REMOVED TO
                                                           // INCREASE
                                                           // TRANSPARENCY
                                                           // QUALITY !!!
          {
            ...
            // TO REMOVE
            Vertex wholeCenter = {0.f, 0.f, postProcessingInfo.param2};
            float distanceToCenter = length(intersection - wholeCenter);
            if (primitive.materialId % 4 != 0)
              i &= (distanceToCenter > postProcessingInfo.param1);
// TO REMOVE
#endif // 0
...
processShadows(const SceneInfo &sceneInfo, BoundingBox *boudingBoxes,
  ...
  computeRayAttributes(r);
  float minDistance =
      sceneInfo.viewDistance; // length(r.direction); //(iteration<2) ?
                              // sceneInfo.viewDistance :
                              // sceneInfo.viewDistance/(iteration+1);

  while (result < (sceneInfo.shadowIntensity - sceneInfo.backgroundColor.w) &&
        ...
        if (primitive.index != lightId && primitive.index != objectId &&
          ...
          if (hit) {
            ...
            if (l > EPSILON && l < length(O_L)) // ??? 0.f
            {
                     ...
                     if( false && materials[primitive.materialId].transparency != 0.f )
                     {
                        O_L=normalize(O_L);
                        ...
                        ratio *= r*a;
                        // Shadow color
                        color.x  += ratio*(0.1f-0.1f*materials[primitive.materialId].color.x);
                        color.y  += ratio*(0.1f-0.1f*materials[primitive.materialId].color.y);
                        color.z  += ratio*(0.1f-0.1f*materials[primitive.materialId].color.z);
                     }
                     result += ratio;
#else
              result +=
                  (materials[primitive.materialId].transparency == 0.f ? 1.f
                                                                       : 0.2f);
              if (materials[primitive.materialId].transparency != 0.f)
                color += 0.2f *
                         (1.f - materials[primitive.materialId].transparency) *
                         materials[primitive.materialId].color;
#endif // 0
            }
          }
        }
  ...
  result =
      (result > sceneInfo.shadowIntensity) ? sceneInfo.shadowIntensity : result;
      ...
      if (lightInformation[cptLamp].attribute.x != primitive.index) {
        ...
        if (lightRayLength < m.innerIllumination.z) {
          ...
          if (material.attributes.z == 2) {
            ...
            if (shadowIntensity < 1.f && sceneInfo.graphicsLevel > 0) {
          ...
          if (dist > postProcessingInfo.param1) {
            ...
            if (sceneInfo.graphicsLevel != 0) {
              ...
              color = primitiveShader(
                  ...
                  box.startIndex + cptPrimitives, intersection, areas,
