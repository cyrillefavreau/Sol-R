...
________________________________________________________________________________
*/
__device__ __INLINE__ float4 skyboxMapping(const SceneInfo &sceneInfo, Material *materials, BitmapBuffer *textures,
                                           const Ray &ray)
{
...
__device__ __INLINE__ bool planeIntersection(const SceneInfo &sceneInfo, const Primitive &primitive,
                                             ...
                                             Vertex &intersection, Vertex &normal, float &shadowIntensity, bool reverse)
{
...
__device__ __INLINE__ bool triangleIntersection(const SceneInfo &sceneInfo, const Primitive &triangle, const Ray &ray,
                                                Vertex &intersection, Vertex &normal, Vertex &areas,
...
________________________________________________________________________________
*/
__device__ float4 intersectionShader(const SceneInfo &sceneInfo, const Primitive &primitive, Material *materials,
                                     BitmapBuffer *textures, Vertex &intersection, const Vertex &areas, Vertex &normal,
                                     Vertex &specular, Vertex &attributes, Vertex &advancedAttributes)
{
...
__device__ __INLINE__ bool intersectionWithPrimitives(
    ...
    BitmapBuffer *textures, const Ray &ray, const int &iteration, int &closestPrimitive, Vertex &closestIntersection,
    Vertex &closestNormal, Vertex &closestAreas, float4 &closestColor, float4 &colorBox, const int currentmaterialId)
{
...
__device__ __INLINE__ float processShadows(const SceneInfo &sceneInfo, BoundingBox *boudingBoxes,
                                           ...
                                           BitmapBuffer *textures, const int &nbPrimitives, const Vertex &lampCenter,
                                           ...
                                           float4 &color, const int &objectId)
{
    ...
    computeRayAttributes(r);
    float minDistance = sceneInfo.viewDistance; // length(r.direction); //(iteration<2) ?
                                                // sceneInfo.viewDistance :
                                                // sceneInfo.viewDistance/(iteration+1);

    while (result < (sceneInfo.shadowIntensity - sceneInfo.backgroundColor.w) && cptBoxes < nbActiveBoxes)
    {
                    ...
                    if (hit)
                    {
                     ...
                     result += ratio;
                            ...
                            if (materials[primitive.materialId].transparency != 0.f)
                                color += 0.2f * (1.f - materials[primitive.materialId].transparency) *
...
__device__ __INLINE__ float4 primitiveShader(
    ...
    BitmapBuffer *textures, RandomBuffer *randoms, const Vertex &origin, Vertex &normal, const int &objectId,
    Vertex &intersection, const Vertex &areas, float4 &closestColor, const int &iteration, float4 &refractionFromColor,
    float &shadowIntensity, float4 &totalBlinn, Vertex &attributes)
{
        ...
        for (int cpt = 0; cpt < lightInformationSize; ++cpt)
        {
            ...
            if (lightInformation[cptLamp].attribute.x != primitive.index)
            {
                ...
                if (lightRayLength < m.innerIllumination.z)
                {
                    ...
                    {
                        lampsColor += (1.f - shadowIntensity) * lightInformation[cptLamp].color;
                    }
                    else
                    {
...
________________________________________________________________________________
*/
__device__ __INLINE__ void makeColor(const SceneInfo &sceneInfo, float4 &color, BitmapBuffer *bitmap, int index)
{
