        ...
        PrimitiveXYIdBuffer& primitiveXYId)
{
        ...
        if( carryon )
        {
            ...
            if( iteration==0 )
            {
                ...
                if( materials[primitives[closestPrimitive].materialId].innerIllumination.x==0.f &&
                        (sceneInfo.parameters.z==aiGlobalIllumination || sceneInfo.parameters.z==aiAdvancedGlobalIllumination ))
                {
                    // Global illumination
        ...
        if( useGlobalIllumination && sceneInfo.parameters.z==aiAdvancedGlobalIllumination )
        {
                        ...
                        normal, areas, closestColor, colorBox, MATERIAL_NONE))
            {
                if(primitives[closestPrimitive].materialId!=MATERIAL_NONE)
                {
                    ...
                    {
                        //colors[0]=material.color*0.3f*pathTracingRatio;
    ...
    if( depthOfField<postProcessingInfo.param1 && materials[primitives[closestPrimitive].materialId].innerIllumination.x==0.f)
    {
        ...
        PrimitiveXYIdBuffer*  primitiveXYIds)
{
    ...
    if( sceneInfo.pathTracingIteration >= NB_MAX_ITERATIONS )
    {
        ...
        ray.direction.y += randoms[rindex+1]*postProcessingBuffer[index].colorInfo.w*postProcessingInfo.param2*a;
        ray.direction.z += randoms[rindex+2]*postProcessingBuffer[index].colorInfo.w*postProcessingInfo.param2*a;
    }
...
#define NB_FILTERS 6
    ...
    if(postProcessingInfo.param3<NB_FILTERS)
    {
        ...
        color.x += min(max(filterFactors[postProcessingInfo.param3].x*localColor.x+filterFactors[postProcessingInfo.param3].y/255.f, 0.f), 1.f);
        color.y += min(max(filterFactors[postProcessingInfo.param3].x*localColor.y+filterFactors[postProcessingInfo.param3].y/255.f, 0.f), 1.f);
        color.z += min(max(filterFactors[postProcessingInfo.param3].x*localColor.z+filterFactors[postProcessingInfo.param3].y/255.f, 0.f), 1.f);
    }
        ...
        SceneInfo sceneInfo )
{
    for( int device(0); device<occupancyParameters.x; ++device )
    {
        ...
        LOG_INFO(1, "Total variable GPU memory allocated on device " << device << ": " << totalMemoryAllocation << " bytes" );
    }
}
