  ...
  while (cptBoxes < m_nbActiveBoxes[m_frame]) {
    ...
    if (boxIntersection(box, r, 0.f, minDistance)) {
      ...
      if (m_sceneInfo.renderBoxes != 0) {
        colorBox.x +=
            m_hMaterials[box.startIndex % NB_MAX_MATERIALS].color.x / 50.f;
        colorBox.y +=
            m_hMaterials[box.startIndex % NB_MAX_MATERIALS].color.y / 50.f;
        colorBox.z +=
            m_hMaterials[box.startIndex % NB_MAX_MATERIALS].color.z / 50.f;
      } else {
...
float CPUKernel::processShadows(const Vertex &lampCenter,
          ...
          if (hit) {
            ...
            if (l > EPSILON && l < vectorLength(O_L)) {
              ...
              if (m_hMaterials[primitive.materialId].transparency != 0.f) {
                ...
                float r =
                    (m_hMaterials[primitive.materialId].transparency == 0.f)
                        ? 1.f
                        : (1.f -
                           0.8f *
                ...
                color.x +=
                    ratio *
                    (0.3f - 0.3f * m_hMaterials[primitive.materialId].color.x);
                color.y +=
                    ratio *
                    (0.3f - 0.3f * m_hMaterials[primitive.materialId].color.y);
                color.z +=
                    ratio *
                    (0.3f - 0.3f * m_hMaterials[primitive.materialId].color.z);
              }
              result += ratio;
            }
            it++;
  ...
  result = (result > m_sceneInfo.shadowIntensity) ? m_sceneInfo.shadowIntensity
                                                  : result;
    ...
    for (int cpt = 0; cpt < nbLamps; ++cpt) {
      ...
      if (li.attribute.x != primitive.index) {
        ...
        if (li.attribute.x >= 0 &&
          ...
          center.x += m_hMaterials[lamp.materialId].innerIllumination.y *
                      m_hRandoms[t] * m_sceneInfo.pathTracingIteration /
                      float(m_sceneInfo.maxPathTracingIterations);
          center.y += m_hMaterials[lamp.materialId].innerIllumination.y *
                      m_hRandoms[t + 1] * m_sceneInfo.pathTracingIteration /
                      float(m_sceneInfo.maxPathTracingIterations);
          center.z += m_hMaterials[lamp.materialId].innerIllumination.y *
        ...
        if (m_sceneInfo.graphicsLevel > 0) {
          ...
          // Lambert
          // --------------------------------------------------------------------------------
          float lambert = (m_postProcessingInfo.type == ppe_ambientOcclusion)
                              ? 0.6f
                              : dot(normal, lightRay);
...
#endif // PHOTON_ENERGY
  {
    ...
    if (carryon) {
      ...
      previousTransparency =
          back ? 1.f : m_hMaterials[m_hPrimitives[closestPrimitive].materialId]
                           .transparency;
#endif // PHOTON_ENERGY
      ...
      if (m_sceneInfo.pathTracingIteration != 0 &&
          m_hMaterials[m_hPrimitives[closestPrimitive].materialId].color.w !=
              0.f) {
        // Randomize view
        float ratio =
            m_hMaterials[m_hPrimitives[closestPrimitive].materialId].color.w;
        ratio *= (m_hMaterials[m_hPrimitives[closestPrimitive].materialId]
                      .transparency == 0.f)
                     ? 1000.f
                     : 1.f;
      ...
      if (m_sceneInfo.pathTracingIteration == 0) {
        ...
        if (pathTracing) {
          ...
          lightRay.direction.x +=
              1000.f * m_hRandoms[rindex] * m_postProcessingInfo.param1;
          lightRay.direction.y +=
              1000.f * m_hRandoms[rindex + 1] * m_postProcessingInfo.param1;
          lightRay.direction.z +=
              1000.f * m_hRandoms[rindex + 2] * m_postProcessingInfo.param1;
...
#pragma omp parallel for
  for (x = 0; x < m_sceneInfo.size.x; ++x) {
    for (int y(0); y < m_sceneInfo.size.y; ++y) {
      ...
      if (m_sceneInfo.pathTracingIteration > NB_MAX_ITERATIONS) {
        localColor.x /=
            (float)(m_sceneInfo.pathTracingIteration - NB_MAX_ITERATIONS + 1);
        localColor.y /=
            (float)(m_sceneInfo.pathTracingIteration - NB_MAX_ITERATIONS + 1);
        localColor.z /=
            (float)(m_sceneInfo.pathTracingIteration - NB_MAX_ITERATIONS + 1);
